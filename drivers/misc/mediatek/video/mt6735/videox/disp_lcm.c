...
#include "hwinfo.h"
struct hwinfo * mtkfb_hwinfo = NULL;
#endif
int g_lcm_status=1; //wcl add 10.9
...
#ifdef CONFIG_HQ_HWINFO
	mtkfb_hwinfo = hwinfo_create("lcm");
       if(!mtkfb_hwinfo)
              printk("hwinfo create fail\r\n");
#endif

	DISPPRINT("%s\n", __func__);
	DISPPRINT("plcm_name=%s\n", plcm_name);
	if (_lcm_count() == 0) {
...
#ifdef CONFIG_HQ_HWINFO
				hwinfo_set_devinfo(mtkfb_hwinfo,"%s",lcm_drv->name);
				if(hwinfo_register(mtkfb_hwinfo))
...
{
	LCM_DRIVER *lcm_drv = NULL;
    g_lcm_status=0;//wcl add 10.9
#ifdef CONFIG_LOG_JANK
   LOG_JANK_D(JLID_KERNEL_LCD_POWER_OFF, "%s", "JLID_KERNEL_LCD_POWER_OFF");
#endif
	DISPFUNC();
	if (_is_lcm_inited(plcm)) {
		lcm_drv = plcm->drv;
	...
	printk("lcm suspend: g_lcm_status = %d", g_lcm_status);
	DISPERR("lcm_drv is null\n");
	return -1;
}
...
int disp_lcm_resume(disp_lcm_handle *plcm)
{
	...
	printk("lcm resume: g_lcm_status = %d", g_lcm_status);
