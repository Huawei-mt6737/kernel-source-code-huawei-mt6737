...
unsigned int _need_do_esd_check(void)
{
	...
	char *br_ptr = NULL;
	int length = 0;
	br_ptr = strstr(saved_command_line, "isNotNeedESDCheck=");
	if (NULL != br_ptr) {
		length = strlen("isNotNeedESDCheck=");
		if(*(br_ptr+length) == '1')
			return 0;
	}
...
extern int s6e8aa5x01_dsm_report_err(int errno);
#endif
...
#ifdef CONFIG_HUAWEI_DSM
					s6e8aa5x01_dsm_report_err(DSM_LCD_ESD_CHECK_ERROR_NO);
#endif
...
int primary_display_disable_ovl2mem(void)
{
	...
	if (_is_decouple_mode(pgc->session_mode) &&
		...
		DISPCHECK("[POWER]stop cmdq[begin]\n");
		_cmdq_stop_trigger_loop();
		DISPCHECK("[POWER]stop cmdq[end]\n");
