...
#ifdef CONFIG_HUAWEI_DSM
	unsigned int  dsi_status;
	unsigned int  dsi_status_test;
	unsigned int  dsi_status_ack;
#endif

	for (d = DSI_MODULE_BEGIN(module); d <= DSI_MODULE_END(module); d++) {
		if (DSI_REG[d]->DSI_MODE_CTRL.MODE)	{
#ifdef CONFIG_HUAWEI_DSM
				s6e8aa5x01_dsm_report_err(DSM_LCD_MIPI_CMD_ERROR_NO);
#endif
				return 0;
			}
		...
		do {
			...
			if (timeout == 1000) {
#ifdef CONFIG_HUAWEI_DSM
				DSI_OUTREG32(cmdq, &dsi_status, AS_UINT32(&DSI_REG[d]->DSI_STA));
				s6e8aa5x01_dsm_report_lcd_status_err(DSM_LCD_LDI_UNDERFLOW_ERROR_NO,dsi_status);
#endif
				ret = 0;
			}
...
#ifdef CONFIG_HUAWEI_DSM
				DSI_OUTREG32(cmdq, &dsi_status_test, AS_UINT32(&DSI_REG[d]->DSI_STA));
				s6e8aa5x01_dsm_report_lcd_status_err(DSM_LCD_LDI_UNDERFLOW_ERROR_NO,dsi_status_test);
#endif
				DSI_DumpRegisters(module, 2);
			...
			if (ret < 0) {
#ifdef CONFIG_HUAWEI_DSM
				DSI_OUTREG32(cmdq, &dsi_status_ack, AS_UINT32(&DSI_REG[d]->DSI_STA));
				s6e8aa5x01_dsm_report_lcd_status_err(DSM_LCD_LDI_UNDERFLOW_ERROR_NO,dsi_status_ack);
#endif
				return 0;
			}
...
printk("esd_check: %#x, %#x, %#x, %#x\n", read_data1.byte0, dsi_params->lcm_esd_check_table[i].para_list[0], read_data1.byte1,dsi_params->lcm_esd_check_table[i].para_list[1]);
				ret = 0; // esd pass
			}
			else
			{
printk("esd_check1: %#x, %#x, %#x, %#x\n", read_data1.byte0, dsi_params->lcm_esd_check_table[i].para_list[0], read_data1.byte1,dsi_params->lcm_esd_check_table[i].para_list[1]);
				ret = 1; // esd fail
				break;
			}
		}
