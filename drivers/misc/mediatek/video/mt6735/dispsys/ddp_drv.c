...
#define CLIENT_NAME_LCD_S6E8AA5X01		"dsm_lcd"
static int rgb_s6e8aa5x01_debug_mask= 1;
static struct dsm_dev dsm_lcd_s6e8aa5x01 = {
	.name 		= CLIENT_NAME_LCD_S6E8AA5X01,		// dsm client name
	.fops 		= NULL,						      // options
	.buff_size 	= DSM_SENSOR_BUF_MAX,			// buffer size
};
#define S6E8AA5X01_ERR(x...) do {\
    if (rgb_s6e8aa5x01_debug_mask >=0) \
        printk(KERN_ERR x);\
    } while (0)

#define S6E8AA5X01_INFO(x...) do {\
    if (rgb_s6e8aa5x01_debug_mask >=1) \
        printk(KERN_ERR x);\
    } while (0)
#define S6E8AA5X01_FLOW(x...) do {\
    if (rgb_s6e8aa5x01_debug_mask >=2) \
        printk(KERN_ERR x);\
    } while (0)
struct dsm_client *s6e8aa5x01_lcd_dclient = NULL;
int s6e8aa5x01_dsm_report_err(int errno)
{
	int size = 0;
	if (FACTORY_BOOT == get_boot_mode() )
	return 0;
	if(dsm_client_ocuppy(s6e8aa5x01_lcd_dclient)){
		/* buffer is busy */
		S6E8AA5X01_ERR("%s: buffer is busy!, errno = %d\n", __func__,errno);
		return -EBUSY;
	}

	S6E8AA5X01_INFO("dsm error, errno = %d \n", errno);

	size = dsm_client_record(s6e8aa5x01_lcd_dclient,"lcd mipi cmd fail,error=%d\n",errno);

	/*if device is not probe successfully or client is null, don't notify dsm work func*/


	dsm_client_notify(s6e8aa5x01_lcd_dclient, errno);


	return size;
}
int s6e8aa5x01_dsm_report_lcd_status_err(int errno,unsigned int data)
{
	int size = 0;
	...
	size = dsm_client_record(s6e8aa5x01_lcd_dclient,"lcd dsi:status:%d\n",data);
...
#ifdef CONFIG_HUAWEI_DSM
	s6e8aa5x01_lcd_dclient = dsm_register_client(&dsm_lcd_s6e8aa5x01);
	if(!s6e8aa5x01_lcd_dclient)
		S6E8AA5X01_ERR("%s: register dsm_dclient failed!!, line = %d\n", __func__,__LINE__);
#endif
...
static int disp_remove(struct platform_device *pdev)
{
	...
	if(s6e8aa5x01_lcd_dclient)
	{
		dsm_unregister_client(s6e8aa5x01_lcd_dclient,&dsm_lcd_s6e8aa5x01);
		s6e8aa5x01_lcd_dclient=NULL;
	}
#endif
