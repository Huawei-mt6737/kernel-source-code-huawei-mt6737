...
#include "ar1335_l13_mipi_sunwin_Sensor.h"

#define PK_DBG_FUNC(fmt, args...)    pr_debug(PFX  fmt, ##args)

/****************************Modify Following Strings for Debug****************************/
#define PFX "ar1335_sunwin_l13_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = AR1335_L13_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
#if defined(AR1335_L13_OTP)
kal_uint16 awb_R_Gr_ratio=1024;
kal_uint16 awb_B_Gr_ratio= 1024;
kal_uint16 awb_Gb_Gr_ratio= 1024;

static void AR1335_L13_otp_read(kal_uint16 otp_type){   // here type 1100 \BB\F2\D5\DF\CA\C7\C6\E4\CB\FBtype 3700
...
static void AR1335_L13_OTP_AUTO_LOAD_LSC(void)
{
	kal_uint16 temp;
	AR1335_L13_otp_read(0x11);

	// enable sc
	temp = read_cmos_sensor_2(0x3780);  //
	write_cmos_sensor_2byte(0x3780,temp|0x8000);  
}
static void AR1335_L13_otp_read_awb(void)
{
	kal_uint16 temp;
	...
	if(temp&0x0008==0x0008)
		{
			LOG_INF("AR1335 read otpm AWB exist !!!!!\n");	
				...
				AR1335_L13_otp_read(0x37); /// read AWB data;
				...
				if((temp ==0xFFFF)&&((0x304A&0x0040)==0x0040))  // if the flag == 0xffff&0x304A[6]==1 then read AWB
					{
						...
						AR1335_L13_otp_read(0x38); /// read awb checksum data;
...
static void sensor_init(void)
{
  ...
  //  write_cmos_sensor_2byte(0x3EB8, 0x010B); 	// DAC_LD_2_3
    ...
    write_cmos_sensor_2byte(0x0300, 0x0003); 	// VT_PIX_CLK_DIV
    ...
    write_cmos_sensor_2byte(0x0340, 0x0c40); 	// FRAME_LENGTH_LINES
    write_cmos_sensor_2byte(0x0202, 0x094c); 	// COARSE_INTEGRATION_TIME
    
    write_cmos_sensor_2byte(0x31B0, 0x0058);   //Frame preamble 4D
    write_cmos_sensor_2byte(0x31B2, 0x002C);   //Line preamble 28
    write_cmos_sensor_2byte(0x31B4, 0x6350);   //MIPI timing0 230E
    write_cmos_sensor_2byte(0x31B6, 0x73AA);   //MIPI timing1 1348
    write_cmos_sensor_2byte(0x31B8, 0x2413);   //MIPI timing2 1C12
    write_cmos_sensor_2byte(0x31BA, 0x1C70);   //MIPI timing3 185B
    write_cmos_sensor_2byte(0x31BC, 0x860B);   //MIPI timing4 8509
...
static void preview_setting(void)
{
 ...
 /// stop_streaming
	...
	temp=read_cmos_sensor_2(0x301A);
	temp = temp|0x4000;
	write_cmos_sensor_2byte(0x301A, temp);  //Corn	
	mDELAY(20);	
...
static void capture_setting(kal_uint16 currefps)
{
  ...
  /// stop_streaming
	...
	write_cmos_sensor_2byte(0x31B4, 0x4392);   //MIPI timing0 2412
	write_cmos_sensor_2byte(0x31B6, 0x43CA);   //MIPI timing1 142A
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
 	    ...
 	    (*sensor_id)++; 
         //   *sensor_id = *sensor_id;
		printk("by likai add  sensor id: 0x%x\n",*sensor_id);
		    ...
		    if(is_sunwin_camera())
                    {      
                       Sunwin_AR1335_read_OTP(); //read otp data
			    return ERROR_NONE;
                    }
		    else 
                           {
...
static kal_uint32 open(void)
{
    ...
    sensor_init();
   // LOG_INF("zhanghainansunwin%x\n", is_sunwin_camera());
   /* if ((imgsensor_info.sensor_id != (sensor_id))||(!is_sunwin_camera()))
        return ERROR_SENSOR_CONNECT_FAIL;
   */
...
UINT32 AR1335_SUNWIN_L13_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
