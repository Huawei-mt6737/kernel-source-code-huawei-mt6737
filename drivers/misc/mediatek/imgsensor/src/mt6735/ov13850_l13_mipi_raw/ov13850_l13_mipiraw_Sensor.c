...
#include "ov13850_l13_mipiraw_Sensor.h"

 
//travies add for otp 
#include "ov13850_l13_otp.h"
//static struct otp_struct otp_ptr={0} ;
//end add
 
/****************************Modify Following Strings for Debug****************************/
#define PFX "OV13850_l13_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = OV13850_L13_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        spin_unlock(&imgsensor_drv_lock);
        do {
			if(0xD800==(read_cmos_sensor(0x300A) << 8))
			{
		            *sensor_id = return_sensor_id();
					(*sensor_id)+=1;
					printk("likai add to check ov13850 sensor_id is :0x%x\n", *sensor_id);
		            if (*sensor_id == imgsensor_info.sensor_id) {
						sensor_init();
						read_ov13850_eeprom_data();
						write_cmos_sensor(0x0100, 0x00);
					...
					printk("travies add ov13850, Read sensor id fail, write id: 0x%x, id:0x%x\n", imgsensor.i2c_write_id,*sensor_id);
...
UINT32 OV13850_L13_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
    /* To Do : Check Sensor status here */
    if (pfFunc!=NULL)
        *pfFunc=&sensor_func;
    return ERROR_NONE;
}    /*    OV13850_L13_MIPI_RAW_SensorInit    */
