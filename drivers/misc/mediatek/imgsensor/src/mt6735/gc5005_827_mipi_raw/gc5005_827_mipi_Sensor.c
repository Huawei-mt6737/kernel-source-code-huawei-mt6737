...
#include "gc5005_827_mipi_Sensor.h"
...
#define PFX "GC5005_827_camera_sensor"
#define LOG_1 LOG_INF("GC5005_827,MIPI 2LANE\n")
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = GC5005_827_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h

    .checksum_value = 0x43886da0,        //checksum value for Camera Auto Test

    .pre = {
        .pclk = 53625000,                //record different mode's pclk
    ...
    .mclk = 13,//mclk value, suggest 24 or 26 for 24Mhz or 26Mhz
...
#define RG_TYPICAL              0x009E
#define BG_TYPICAL              0x009D
...
{	
	...
	write_cmos_sensor(0xf7, 0x01);//01 
	write_cmos_sensor(0xf8, 0x20); //PLL mode2//11
	write_cmos_sensor(0xf9, 0x6a); //aa
...
static void set_max_framerate(UINT16 framerate,kal_bool min_framelength_en)
{
    ...
    frame_length = imgsensor.pclk / framerate * 10 / imgsensor.line_length;//54000000/296*10/906=2013
...
static void set_shutter(kal_uint16 shutter)
{
	...
	{
		write_cmos_sensor(0x07, (Basic_Dummy_Line >> 8) & 0x1F);
		write_cmos_sensor(0x08, Basic_Dummy_Line & 0xFF);
	}
#endif
	//Update Shutter
	write_cmos_sensor(0xfe, 0x00);	
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0xf7, 0x01); //pll enable//01
	write_cmos_sensor(0xf8, 0x20); ///pll mode//11
	write_cmos_sensor(0xf9, 0x6a); //aa
	...
	/*MIPI*/
	write_cmos_sensor(0xfe, 0x03);
	write_cmos_sensor(0x01, 0x17);
	write_cmos_sensor(0x02, 0x55);//33
	...
	write_cmos_sensor(0x26, 0x09); //09  06
	write_cmos_sensor(0x29, 0x04); //06
	write_cmos_sensor(0x2a, 0x0d);
	write_cmos_sensor(0x2b, 0x09); //09  06
...
static kal_uint32 set_max_framerate_by_scenario(MSDK_SCENARIO_ID_ENUM scenario_id, MUINT32 framerate)
{
	...
	write_cmos_sensor(0x86, 0x52);
	write_cmos_sensor(0x8c, 0x90);

    switch (scenario_id) {
        case MSDK_SCENARIO_ID_CAMERA_PREVIEW:
            frame_length = imgsensor_info.pre.pclk / framerate * 10 / imgsensor_info.pre.linelength;//54000000/300*10/906=1986
            spin_lock(&imgsensor_drv_lock);
            imgsensor.dummy_line = (frame_length > imgsensor_info.pre.framelength) ? (frame_length - imgsensor_info.pre.framelength) : 0;// 2
            imgsensor.frame_length = imgsensor_info.pre.framelength + imgsensor.dummy_line;//1984+2=1986
            imgsensor.min_frame_length = imgsensor.frame_length;
            spin_unlock(&imgsensor_drv_lock);
            set_dummy();
			Basic_Dummy_Line = imgsensor.min_frame_length - 1968;//travis
            break;
        case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
            if(framerate == 0)
                return ERROR_NONE;
...
UINT32 GC5005_827_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
