...
#include "ov8856qtmipiraw_Sensor.h"
...
OV8856QT_VERSION ov8856qtversion = OV8856QTR1A;

static DEFINE_SPINLOCK(imgsensor_drv_lock);

static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV8856QT_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
	
	.checksum_value =  0xb1893b4f,//0xd14be45,
...
{
 { 3296, 2480,    0,	12, 3296, 2456, 3296, 2456,   4,	2, 3264, 2448,	 0, 0, 3264, 2448}, // Preview 
...
int ov8856qt_get_module_id(void)
{
	int otp_flag, addr;
	int module_id = 0 ; 
	...
	if(addr != 0)
	{
	  module_id = read_cmos_sensor(addr);
	}
	else 
	{
	  module_id = 0;
...
static struct otp_struct otp_ptr={0} ;
...
static int read_otp(struct otp_struct *otp_ptr)
{
   ...
   // OTP base information and WB calibration data
   otp_flag = read_cmos_sensor(0x7010);
   LOG_INF("ov8856qt_otp_read_test  otp_flag: 0x%x,\n", otp_flag); 
   addr = 0;
   ...
   if(addr != 0)
   {
     ...
     (*otp_ptr).lens_id = read_cmos_sensor( addr + 1) & 0xFF;
     (*otp_ptr).vcm_id = read_cmos_sensor( addr + 1) >> 4;
   ...
   LOG_INF("ov8856qt_otp_read_test  (*otp_ptr).module_integrator_id: 0x%x,\n", (*otp_ptr).module_integrator_id); 
 // OTP Lenc Calibration
   otp_flag = read_cmos_sensor(0x702c);
 LOG_INF("ov8856qt_otp_read_test	otp_flag_3: 0x%x,\n", otp_flag); 
   addr = 0;
   int checksum2=0;
   ...
   if(addr != 0) 
   {
     ...
     checksum2 = (checksum2)%255 +1;
	  (* otp_ptr).checksum = read_cmos_sensor((addr + 240));
        LOG_INF("ov8856qt_otp_read_test  checksum2: 0x%x,\n",checksum2 );
	   LOG_INF("ov8856qt_otp_read_test  (* otp_ptr).checksum : 0x%x,\n",(* otp_ptr).checksum );
...
*
*************************************************************************/
extern int read_ov8856qt_eeprom_data(void);
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id) 
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		spin_unlock(&imgsensor_drv_lock);
		do {
			if(0x8800==(read_cmos_sensor(0x300B)<< 8)){
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
                           module_id = ov8856qt_get_module_id();
			 //module_id=read_cmos_sensor(0x7011);
			if (module_id==0X06)
				{
					(*sensor_id)++;
					read_ov8856qt_eeprom_data();
				}
		     LOG_INF("OV8856QT sensor id: 0x%x\n", *sensor_id);
		     //LOG_INF("OV8856QT  imgsensor_info_sensor id: 0x%x\n", imgsensor_info.sensor_id);
		     LOG_INF("OV8856QT module id: 0x%x\n",module_id);
			if (*sensor_id == imgsensor_info.sensor_id) {
			//if ((*sensor_id == imgsensor_info.sensor_id)) {
				//if((read_cmos_sensor(0x302A)) == 0XB0){
// *sensor_id ++;
				read_otp(&otp_ptr);//lili add	
				
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x, ov8856qtversion = %d(0=r2a,1=r1a)\n", imgsensor.i2c_write_id,*sensor_id,ov8856qtversion);	
				return ERROR_NONE;
		 }	
		}
	...
	if (*sensor_id != imgsensor_info.sensor_id||module_id!=0x06) {
...
UINT32 OV8856QT_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
