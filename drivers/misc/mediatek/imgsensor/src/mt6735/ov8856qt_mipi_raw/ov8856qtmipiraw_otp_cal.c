...
#define PFX "OV8856QT_EEPROM_FMT"
...
*
********************************************************************************/
#define CAM_CAL_DRVNAME "OV8856QTMIPIRAW_OTP_CAL"
/*******************************************************************************
*
********************************************************************************/
...
unsigned short ov8856qtsy_af_infinit;
unsigned short ov8856qtsy_af_macro;


static OTP_DATA ov8856qt_eeprom_data = {{0}};
...
static int read_ov8856qt_eeprom(u8 slv_id, u16 offset, u8* data)
{
	int ret = 0;
	ret = read_cmos_sensor(slv_id,offset,data);
	return ret;
}
static int read_ov8856qt_eeprom_size(u8 slv_id, u16 offset, u8* data,int size)
{
	int i = 0;
	for(i = 0; i < size; i++){
		if(read_ov8856qt_eeprom(slv_id, offset+i, data+i) != 0)
			return -1;
	}
	return 0;
}
int read_ov8856qt_module_id(void)
{
	int module_id =0;
	read_ov8856qt_eeprom_size(0xA0,0x0003,&module_id,1);
	return module_id;
}

extern kal_uint16 read_cmos_sensor_af(kal_uint32 addr);
...
//        CAM_CALDB("HQ_OV_OTP] ==================================\n");
//        ov8856qt_eeprom_data.Data[0]= 0x01; 
//	ov8856qt_eeprom_data.Data[1]= 0xFF; 
//	ov8856qt_eeprom_data.Data[2]= 0x00; 
//	ov8856qt_eeprom_data.Data[3]= 0x0B; 
//	ov8856qt_eeprom_data.Data[4]= 0x01; 
//return 1;
//} 
//#endif 

//lujun add af
int read_ov8856qt_AF_info(void)
{
	sensor_init();
   ...
   // OTP VCM Calibration
   otp_flag = read_cmos_sensor_af(0x7023);
   
   LOG_INF("ov8856qt_otp_read_test  otp_flag_2: 0x%x,\n", otp_flag); 
   addr = 0;
   ...
   af_data=af_data_info[i];
             LOG_INF("lujun_ov8856qt_otp_read_test  af_data_info: 0x%x,\n",  af_data); 
   }
  ...
  ov8856qt_eeprom_data.Data[2]= 0x00; 
  ov8856qt_eeprom_data.Data[3]= 0x0B; 
  ov8856qt_eeprom_data.Data[4]= 0x02; 
ov8856qt_eeprom_data.Data[17]=(af_data_info[0]<<2)+((af_data_info[2]>>6)&0x03);
ov8856qt_eeprom_data.Data[18]=((((af_data_info[0]<<2)+((af_data_info[2]>>6)&0x03))>>8)&0x03);
ov8856qt_eeprom_data.Data[19]=(af_data_info[1]<<2)+((af_data_info[2]>>4)&0x03);
ov8856qt_eeprom_data.Data[20]=((((af_data_info[1]<<2)+((af_data_info[2]>>4)&0x03))>>8)&0x03);
   LOG_INF("lujun_ov8856qt_otp_read_test  data[17]: 0x%x,\n",  ov8856qt_eeprom_data.Data[17]); 
   LOG_INF("lujun_ov8856qt_otp_read_test  data[18]: 0x%x,\n", ov8856qt_eeprom_data.Data[18]); 
   LOG_INF("lujun_ov8856qt_otp_read_test  data[19]: 0x%x,\n", ov8856qt_eeprom_data.Data[19]); 
   LOG_INF("lujun_ov8856qt_otp_read_test  data[20]: 0x%x,\n", ov8856qt_eeprom_data.Data[20]); 
   }	
   return 0;
}
#if 1
int read_ov8856qt_eeprom_data(void){
...
//	if(EEPROM_Slim_File_Load())
	...
	read_ov8856qt_AF_info();
do_gettimeofday(&ktv2);
	if(ktv2.tv_sec > ktv1.tv_sec)
                TimeIntervalUS = ktv1.tv_usec + 1000000 - ktv2.tv_usec;
        else
                TimeIntervalUS = ktv2.tv_usec - ktv1.tv_usec;
        printk("lujun>>ov8856qt>read data take %lu us\n", TimeIntervalUS);
	return 1;
}
...
static long ov8856qt_Ioctl_Compat(struct file *filp, unsigned int cmd, unsigned long arg)
{
    long ret;
    COMPAT_stCAM_CAL_INFO_STRUCT __user *data32;
    stCAM_CAL_INFO_STRUCT __user *data;
    int err;
	CAM_CALDB("[CAMERA SENSOR] ov8856qt_eeprom_DEVICE_ID,%p %p %x ioc size %d\n",filp->f_op ,filp->f_op->unlocked_ioctl,cmd,_IOC_SIZE(cmd) );
...
static int selective_read_region(u32 offset, BYTE* data,u32 size)
{
	...
	printk("lujun>>ov8856qt>selective_read_region offset = %d,size = %d\n", offset,size);
         memcpy((void *)data,(void *)&ov8856qt_eeprom_data.Data[offset],size);
     	 return size;
}
...
#define NEW_UNLOCK_IOCTL
	...
	static const struct file_operations g_stCAM_CAL_fops =
	{
		...
		//.ioctl = CAM_CAL_Ioctl
#ifdef CONFIG_COMPAT
		.compat_ioctl = ov8856qt_Ioctl_Compat,
#endif
