...
static imgsensor_info_struct imgsensor_info = { 
	...
	.checksum_value = 0xb1893b4f,//0xd14be45,		//checksum value for Camera Auto Test
...
static int read_otp(struct otp_struct *otp_ptr)
{
 ...
 LOG_INF("ov8856_otp_read_test	otp_flag_3: 0x%x,\n", otp_flag); 
   addr = 0;
   int checksum2=0;
   ...
   if(addr != 0) 
   {
     ...
     checksum2 = (checksum2)%255 +1;
	  (* otp_ptr).checksum = read_cmos_sensor((addr + 240));
        LOG_INF("ov8856_otp_read_test  checksum2: 0x%x,\n",checksum2 );
	   LOG_INF("ov8856_otp_read_test  (* otp_ptr).checksum : 0x%x,\n",(* otp_ptr).checksum );
...
static int RG_Ratio_Typical= 0x13e ; //0x14C
static int BG_Ratio_Typical= 0x14d ; //0x130
...
*
*************************************************************************/
extern int read_ov8856_eeprom_data(void);
		...
		do {
			if(0x8800==(read_cmos_sensor(0x300B) << 8))
				...
				*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
				module_id=ov8856_get_module_id();
				//module_id=read_cmos_sensor(0x7011);
				 LOG_INF("OV8856 sensor id: 0x%x\n", *sensor_id);
			         LOG_INF("OV8856 module id: 0x%x\n",module_id);
				if ((*sensor_id == imgsensor_info.sensor_id)&&(module_id==0X08)){
					read_otp(&otp_ptr);//lili add	
					read_ov8856_eeprom_data();
