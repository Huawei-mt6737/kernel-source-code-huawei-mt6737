...
#define LP_OWN_REQ_CLR_INTERVAL_MS   200	/* exponential of 2 */
	...
	while (1) {
		...
		if (u4RegValue & WHLPCR_FW_OWN_REQ_SET) {
			...
			DBGLOG(NIC, WARN, "fgIsBusAccessFailed(%u),fgIsResetting(%u)\n",
			fgIsBusAccessFailed, fgIsResetting);
			...
			if ((i == 0) || (u4WriteTickTemp - u4WriteTick) > LP_OWN_REQ_CLR_INTERVAL_MS) {
