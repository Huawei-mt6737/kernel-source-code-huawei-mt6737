...
#define DSM_BT_MOD_NAME         "dsm_bluetooth"
#define DSM_BT_BUF_SIZE			4096
static struct dsm_dev bt_dsm_info = {
    .name = DSM_BT_MOD_NAME,
    .fops = NULL,
    .buff_size = DSM_BT_BUF_SIZE,
};

static struct dsm_client *bt_dclient = NULL;

int bt_dsm_register(void)
{
    if (NULL != bt_dclient)
    {
        printk(KERN_INFO "bt_dclient had been register!\n");
        return 0;
    }

    bt_dclient = dsm_register_client(&bt_dsm_info);
    if(NULL == bt_dclient)
    {
        printk(KERN_ERR "bt_dclient register failed!\n");
    }

    return 0;
}
EXPORT_SYMBOL(bt_dsm_register);

int bt_dsm_report_num(int dsm_err_no, char *err_msg, int err_code)
{
    int err = 0;

    if(NULL == bt_dclient)
    {
        printk(KERN_ERR "%s bt_dclient did not register!\n", __func__);
        return 0;
    }

    err = dsm_client_ocuppy(bt_dclient);
    if(0 != err)
    {
    ...
    err = dsm_client_record(bt_dclient, "err_msg:%s;err_code:%d;\n",err_msg,err_code);
    dsm_client_notify(bt_dclient, dsm_err_no);

    return 0;
}
EXPORT_SYMBOL(bt_dsm_report_num);

int bt_dsm_report_info(int error_no, void *log, int size)
{
    int err = 0;
    int rsize = 0;
    ...
    if((error_no < DSM_BLUETOOTH_DM_ERROR) || (error_no > DSM_BLUETOOTH_BLE_ERROR) || (NULL == log) || (size < 0))
    {
    ...
    if(size > DSM_BT_BUF_SIZE)
    {
        rsize = DSM_BT_BUF_SIZE;
    }
    else
    {
        rsize = size;
    }
    err = dsm_client_copy(bt_dclient, log, rsize);

    dsm_client_notify(bt_dclient, error_no);

    return 0;
}
EXPORT_SYMBOL(bt_dsm_report_info);

#else
int inline bt_dsm_register()
{
    return 0;
}
EXPORT_SYMBOL(bt_dsm_register);
...
int inline bt_dsm_report_info(int error_no, void *log, int size)
{
    return 0;
}
EXPORT_SYMBOL(bt_dsm_report_info);
...
static int BT_open(struct inode *inode, struct file *file)
{
	...
	if (MTK_WCN_BOOL_FALSE == mtk_wcn_wmt_func_on(WMTDRV_TYPE_BT)) {
        ...
        #ifdef CONFIG_HUAWEI_DSM
            bt_dsm_report_num(DSM_BLUETOOTH_BLE_CONNECT_ERROR, "power on failed", -1);
        #endif
		return -ENODEV;
	}
	...
	if (mtk_wcn_stp_is_ready()) {
        ...
        #ifdef CONFIG_HUAWEI_DSM
            bt_dsm_report_num(DSM_BLUETOOTH_HFP_CONNECT_ERROR, "uart open/close failed", -1);
        #endif
		return -ENODEV;
	}
...
static void BT_exit(void)
{
	dev_t dev = MKDEV(BT_major, 0);
#ifdef CONFIG_HUAWEI_DSM
    dsm_unregister_client(bt_dclient,&bt_dsm_info);
#endif
#if WMT_CREATE_NODE_DYNAMIC
	if (stpbt_dev) {
		device_destroy(stpbt_class, dev);
