...
#define CLIENT_NAME_SENSOR_BMA253		"dsm_sensor_bma253"
static int rgb_bma253_debug_mask= 1;
static struct dsm_dev dsm_sensor_bma253 = {
	.name 		= CLIENT_NAME_SENSOR_BMA253,		// dsm client name
	.fops 		= NULL,						      // options
	.buff_size 	= DSM_SENSOR_BUF_MAX,			// buffer size
};
#define BMA253_ERR(x...) do {\
    if (rgb_bma253_debug_mask >=0) \
        printk(KERN_ERR x);\
    } while (0)

#define BMA253_INFO(x...) do {\
    if (rgb_bma253_debug_mask >=1) \
        printk(KERN_ERR x);\
    } while (0)
#define BMA253_FLOW(x...) do {\
    if (rgb_bma253_debug_mask >=2) \
        printk(KERN_ERR x);\
    } while (0)
static struct dsm_client *bma253_sensor_dclient = NULL;
static int bma253_dsm_report_err(int errno,int type)
{
	int size = 0;

	if (FACTORY_BOOT == get_boot_mode()||(bma253_sensor_dclient == NULL) )
	return 0;

	if(dsm_client_ocuppy(bma253_sensor_dclient)){
		/* buffer is busy */
		BMA253_ERR("%s: buffer is busy!, errno = %d\n", __func__,errno);
		return -EBUSY;
	}

	BMA253_INFO("dsm error, errno = %d \n", errno);

 	if(type == -EINVAL){
	size = dsm_client_record(bma253_sensor_dclient,"CLIENT CANN'T EQUAL NULL ,i2c erro type:%d\n",type);
 		}
	if(type == -EIO){
	size = dsm_client_record(bma253_sensor_dclient,"i2c_transfer error ,i2c erro type:%d\n",type);
 		}
	if(type == -EFAULT){
	size = dsm_client_record(bma253_sensor_dclient,"send command error!! ,i2c erro type:%d\n",type);
 		}

	/*if device is not probe successfully or client is null, don't notify dsm work func*/


	dsm_client_notify(bma253_sensor_dclient, errno);
...
static int bma_i2c_read_block(struct i2c_client *client, u8 addr, u8 *data, u8 len)
{
...
#ifdef CONFIG_HUAWEI_DSM
		bma253_dsm_report_err(DSM_GSENSOR_I2C_BUS_ERROR_NO,-EINVAL);
#endif
	...
	{
#ifdef CONFIG_HUAWEI_DSM
	bma253_dsm_report_err(DSM_GSENSOR_I2C_BUS_ERROR_NO,-EIO);
#endif
...
{   /*because address also occupies one byte, the maximum length for write is 7 bytes*/
	...
	if (err < 0)
	{
		...
		mutex_unlock(&gsensor_mutex);
#ifdef CONFIG_HUAWEI_DSM
	bma253_dsm_report_err(DSM_GSENSOR_I2C_BUS_ERROR_NO,-EFAULT);
#endif
		return -EFAULT;
	} 
	mutex_unlock(&gsensor_mutex);
...
#endif
/* End note by wuxiaolin 2016-06-23*/
...
static int BMA2XX_SetPowerMode(struct i2c_client *client, bool enable)
{
	...
	if(enable == TRUE)
	{
		...
		udelay(1000);
		res = bma_i2c_read_block(client,BMA2XX_REG_POWER_CTL, &temp0, 0x1);
		if (res < 0)
...
#ifdef CONFIG_HUAWEI_DSM
	bma253_sensor_dclient = dsm_register_client(&dsm_sensor_bma253);
	if(!bma253_sensor_dclient)
		BMA253_ERR("%s: register dsm_dclient failed!!, line = %d\n", __func__,__LINE__);
#endif
	return 0;
...
static int bma2xx_i2c_remove(struct i2c_client *client)
{
	...
	if(bma253_sensor_dclient)
		{
	dsm_unregister_client(bma253_sensor_dclient,&dsm_sensor_bma253);
	bma253_sensor_dclient=NULL;
		}
	#endif
...
static int  gsensor_local_init(void)
{
	...
	{
		GSE_ERR("bma2xx gsensor_local_init fail,gsensor_init_flag = %d\n",gsensor_init_flag);
		return -1;
	}
