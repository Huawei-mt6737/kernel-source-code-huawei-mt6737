	...
	if (0 == enable) {
		...
		if (true == cxt->is_polling_run) {
			if (false == cxt->acc_ctl.is_report_input_direct) {
                ...
                cxt->drv_data.acc_data.status = ACC_INVALID_VALUE;
                input_abs_set_val(cxt->idev, EVENT_TYPE_ACCEL_STATUS, cxt->drv_data.acc_data.status);
			}
		}
	acc_real_enable(enable);
	}
...
static int acc_real_driver_init(void)
{
	...
	int err = 0;
#ifdef CONFIG_HQ_HWINFO
	acc_hwinfo = hwinfo_create("accel");
	if(!acc_hwinfo)
		ACC_LOG("hwinfo create fail\r\n");
#endif
	...
	for (i = 0; i < MAX_CHOOSE_G_NUM; i++) {
		...
		if (0 != gsensor_init_list[i]) {
			...
			if (0 == err) {
#ifdef CONFIG_HQ_HWINFO
				hwinfo_set_devinfo(acc_hwinfo,"%s",gsensor_init_list[i]->name);
#endif
...
#ifdef CONFIG_HQ_HWINFO
	if(hwinfo_register(acc_hwinfo))
		ACC_LOG("hwinfo register fail\r\n");
#endif
