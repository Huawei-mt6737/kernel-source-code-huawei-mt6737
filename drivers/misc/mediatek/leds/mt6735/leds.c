...
#define PMIC_ISIN0_MODE ISINK_BREATH_MODE
#define PMIC_ISIN1_MODE ISINK_BREATH_MODE
...
#define PMIC_BREATH_TON_NUM 16
#define PMIC_BREATH_TOFF_NUM 16
...
int pmic_breath_Ton_sel[]={123,338,523,707,926,1107,1291,1507,1691,1876,2091,2276,2460,2676,2860,3075};
int pmic_breath_Toff_sel[]={246,667,1046,1417,1845,2214,2583,3014,3383,3752,4183,4552,4921,5351,5720,6151};
...
static int breath_Ton_time_index_pmic(int time_ms)
{
     int i;
     for (i = 0; i < PMIC_BREATH_TON_NUM; i++) {
         if (time_ms <= pmic_breath_Ton_sel[i])
             return i;
     }
     return PMIC_BREATH_TON_NUM - 1;
}

static int breath_Toff_time_index_pmic(int time_ms)
{

       int i;
       for (i = 0; i < PMIC_BREATH_TOFF_NUM; i++) {
           if (time_ms <= pmic_breath_Toff_sel[i])
              return i;
       }
       return PMIC_BREATH_TOFF_NUM - 1;
}
...
int mt_led_blink_pmic(enum mt65xx_led_pmic pmic_type, struct nled_setting *led)
{
	int time_index = 0;
	int duty = 0;
	int breath_Ton_index=0;
	int breath_Toff_index=0;
	...
	case MT65XX_LED_PMIC_NLED_ISINK0:
		...
		if(PMIC_ISIN0_MODE==ISINK_BREATH_MODE)
		{
		breath_Ton_index=breath_Ton_time_index_pmic(led->blink_on_time-200);//breath time
		breath_Toff_index=breath_Toff_time_index_pmic(led->blink_off_time-200);//breath time
		LEDS_DEBUG("LED BREATH MODE TON_index = %d,TOFF_index = %d\n",breath_Ton_index,breath_Toff_index);
		pmic_set_register_value(PMIC_ISINK_CH0_MODE,ISINK_BREATH_MODE);
		pmic_set_register_value(PMIC_ISINK_CH0_STEP,ISINK_1);//8mA
		pmic_set_register_value(PMIC_ISINK_BREATH0_TR1_SEL,0x00);
		pmic_set_register_value(PMIC_ISINK_BREATH0_TR2_SEL,0x00);
		pmic_set_register_value(PMIC_ISINK_BREATH0_TF1_SEL,0x00);
		pmic_set_register_value(PMIC_ISINK_BREATH0_TF2_SEL,0x00);
		pmic_set_register_value(PMIC_ISINK_BREATH0_TON_SEL,breath_Ton_index);
		pmic_set_register_value(PMIC_ISINK_BREATH0_TOFF_SEL,breath_Toff_index);
		}else if(PMIC_ISIN0_MODE==ISINK_PWM_MODE){
		time_index = find_time_index_pmic(led->blink_on_time + led->blink_off_time);
		LEDS_DEBUG("LED PWM MODE time_index = %d , freqsel=%d\n", time_index, pmic_freqsel_array[time_index]);
	...
	case MT65XX_LED_PMIC_NLED_ISINK1:
		...
		if(PMIC_ISIN1_MODE==ISINK_BREATH_MODE)
		{
		breath_Ton_index=breath_Ton_time_index_pmic(led->blink_on_time-200);//breath time
		breath_Toff_index=breath_Toff_time_index_pmic(led->blink_off_time-200);//breath time
		LEDS_DEBUG("LED BREATH MODE TON_index is %d,TOFF_index is %d\n",breath_Ton_index,breath_Toff_index);
		pmic_set_register_value(PMIC_ISINK_CH1_MODE,ISINK_BREATH_MODE);
		pmic_set_register_value(PMIC_ISINK_CH1_STEP,ISINK_1);//8mA
		pmic_set_register_value(PMIC_ISINK_BREATH1_TR1_SEL,0x00);
		pmic_set_register_value(PMIC_ISINK_BREATH1_TR2_SEL,0x00);
		pmic_set_register_value(PMIC_ISINK_BREATH1_TF1_SEL,0x00);
		pmic_set_register_value(PMIC_ISINK_BREATH1_TF2_SEL,0x00);
		pmic_set_register_value(PMIC_ISINK_BREATH1_TON_SEL,breath_Ton_index);
		pmic_set_register_value(PMIC_ISINK_BREATH1_TOFF_SEL,breath_Toff_index);
		}else if(PMIC_ISIN1_MODE==ISINK_PWM_MODE){
		time_index = find_time_index_pmic(led->blink_on_time + led->blink_off_time);
...
int mt_brightness_set_pmic(enum mt65xx_led_pmic pmic_type, u32 level, u32 div)
{
	...
	u32 temp_level = (6*level)/255;
	LEDS_DEBUG("PMIC#%d=>%d:%d\n", pmic_type,temp_level, level);
	...
	if (pmic_type == MT65XX_LED_PMIC_NLED_ISINK0) {
		...
		pmic_set_register_value(PMIC_RG_DRV_ISINK0_CK_CKSEL, 0);
		pmic_set_register_value(PMIC_ISINK_CH0_MODE, ISINK_PWM_MODE);
		pmic_set_register_value(PMIC_ISINK_CH0_STEP, ISINK_1);	/* 16mA */
		pmic_set_register_value(PMIC_ISINK_DIM0_DUTY, 15);
		...
		pmic_set_register_value(PMIC_RG_DRV_ISINK1_CK_CKSEL, 0);
		pmic_set_register_value(PMIC_ISINK_CH1_MODE, ISINK_PWM_MODE);
		pmic_set_register_value(PMIC_ISINK_CH1_STEP, ISINK_1);	/* 16mA */
		pmic_set_register_value(PMIC_ISINK_DIM1_DUTY, 15);
...
extern     int  FL_enable_sub(void);
extern     int  FL_disable_sub(void);
	...
	case MT65XX_LED_MODE_PWM:
		if (strcmp(cust->name, "lcd-backlight") == 0) {
			...
			bl_duty_hal = level;
              ...
              else   if (strcmp(cust->name, "hq-torch") == 0) {
			if (level == 0) {
			sgm3785_shutoff();
			} 
			else {
	       ...
	       else	 if (strcmp(cust->name, "sub-flashlight") == 0) {
			if (level == 0) {
				FL_disable_sub();
