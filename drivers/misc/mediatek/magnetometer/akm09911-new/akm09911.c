...
#define CLIENT_NAME_SENSOR_AKM09911		"dsm_sensor_akm09911"
static int rgb_akm09911_debug_mask= 1;
static struct dsm_dev dsm_sensor_akm09911 = {
	.name 		= CLIENT_NAME_SENSOR_AKM09911,		// dsm client name
	.fops 		= NULL,						      // options
	.buff_size 	= DSM_SENSOR_BUF_MAX,			// buffer size
};
#define AKM09911_ERR(x...) do {\
    if (rgb_akm09911_debug_mask >=0) \
        printk(KERN_ERR x);\
    } while (0)

#define AKM09911_INFO(x...) do {\
    if (rgb_akm09911_debug_mask >=1) \
        printk(KERN_ERR x);\
    } while (0)
#define AKM09911_FLOW(x...) do {\
    if (rgb_akm09911_debug_mask >=2) \
        printk(KERN_ERR x);\
    } while (0)
static struct dsm_client *akm09911_sensor_dclient = NULL;
static int akm09911_dsm_report_err(int errno)
{
	int size = 0;

	if (FACTORY_BOOT == get_boot_mode()||(akm09911_sensor_dclient == NULL))
	return 0;

	if(dsm_client_ocuppy(akm09911_sensor_dclient)){
		/* buffer is busy */
		AKM09911_ERR("%s: buffer is busy!, errno = %d\n", __func__,errno);
		return -EBUSY;
	}

	AKM09911_INFO("dsm error, errno = %d \n", errno);

	size = dsm_client_record(akm09911_sensor_dclient,
	"i2c_master_send fail, retry over 10 ,i2c erro type:%d\n",-EIO);

	/*if device is not probe successfully or client is null, don't notify dsm work func*/


	dsm_client_notify(akm09911_sensor_dclient, errno);
...
/*------- add by wuxiaolin to add gpio set function -----------*/
static struct pinctrl *pinctrl1;
static struct pinctrl_state *pins_default;
static struct pinctrl_state *rst_output0, *rst_output1;
struct of_device_id mag_platform_of_match[] = {
	{ .compatible = "mediatek,mt6735-mag", },
	{},
};

static DEFINE_MUTEX(mag_set_gpio_mutex);

static void mag_rst_gpio_output(int level);
static int mag_get_gpio_info(struct platform_device *pdev);
...
static struct platform_driver mag_driver = {
	...
	.driver = {
			.name = "AKM09911",
			.owner = THIS_MODULE,
			.of_match_table = mag_platform_of_match,
	},
};
/*---------- end by wuxiaolin to add gpio set function -----------*/
...
/*------- add by wuxiaolin to add gpio set function -----------*/
void mag_rst_gpio_output(int level)
{
	mutex_lock(&mag_set_gpio_mutex);
	
	MAGN_ERR("[mag]mag_rst_gpio_output level = %d\n",level);
	...
	mutex_unlock(&mag_set_gpio_mutex);
}
...
static int mag_get_gpio_info(struct platform_device *pdev)
{
	...
	MAGN_ERR("[mag %d] mt_mag_pinctrl+++++++++++++++++\n", pdev->id);
	pinctrl1 = devm_pinctrl_get(&pdev->dev);
	if (IS_ERR(pinctrl1)) {
		ret = PTR_ERR(pinctrl1);
		dev_err(&pdev->dev, "fwq Cannot find mag pinctrl1!\n");
		return ret;
	}

	pins_default = pinctrl_lookup_state(pinctrl1, "mag_default");
	if (IS_ERR(pins_default)) {
		ret = PTR_ERR(pins_default);
		dev_err(&pdev->dev, "fwq Cannot find mag pinctrl default %d!\n", ret);
	}

	rst_output0 = pinctrl_lookup_state(pinctrl1, "mag_rst_output0");
	if (IS_ERR(rst_output0)) {
		ret = PTR_ERR(rst_output0);
		dev_err(&pdev->dev, "fwq Cannot find mag pinctrl state_rst_output0!\n");
		return ret;
	}
	rst_output1 = pinctrl_lookup_state(pinctrl1, "mag_rst_output1");
	if (IS_ERR(rst_output1)) {
		ret = PTR_ERR(rst_output1);
		dev_err(&pdev->dev, "fwq Cannot find mag pinctrl state_rst_output1!\n");
		return ret;
	}
...
static int mag_probe(struct platform_device *pdev)
{
	mag_get_gpio_info(pdev);
	
	mag_rst_gpio_output(1);
	mdelay(5);
...
#ifdef CONFIG_HUAWEI_DSM
	   akm09911_dsm_report_err(DSM_MSENSOR_I2C_DATA_ERROR_NO);
#endif
...
int FST_AK09916(void)
{
	...
	//int   asay;
	//int   asaz;
        MAGN_LOG("kernel log ok xieyinhao\n");
	/* *********************************************** */
	/* Reset Test Result */
	/* *********************************************** */
	pf_total = 1;
	...
	hdata[0] = (s16)(i2cData[1] | (i2cData[2] << 8))/4;
	hdata[1] = (s16)(i2cData[3] | (i2cData[4] << 8))/4;
	hdata[2] = (s16)(i2cData[5] | (i2cData[6] << 8))/4;
	i2cData[8] = 0;
	/* TEST */
	i2cData[0] &= 0x7F;
	TEST_DATA(TLIMIT_NO_SNG_ST1_09911,  TLIMIT_TN_SNG_ST1_09911,  (int)i2cData[0], TLIMIT_LO_SNG_ST1_09911,
...
int FctShipmntTestProcess_Body(void)
{
	...
	pf_total = FST_AK8963();
	#else
	pf_total = FST_AK09916();//FST_AK09911();
	#endif
...
static int akm09911_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	// add by wuxiaolin for the rst pin need low to high
	mag_rst_gpio_output(0);
	mdelay(5);
	mag_rst_gpio_output(1);
	// end by wuxiaolin
...
#ifdef CONFIG_HUAWEI_DSM
	akm09911_sensor_dclient = dsm_register_client(&dsm_sensor_akm09911);
	if(!akm09911_sensor_dclient)
		AKM09911_ERR("%s: register dsm_dclient failed!!, line = %d\n", __func__,__LINE__);
#endif
...
static int akm09911_i2c_remove(struct i2c_client *client)
{
	...
	if(akm09911_sensor_dclient)
		{
	dsm_unregister_client(akm09911_sensor_dclient,&dsm_sensor_akm09911);
	akm09911_sensor_dclient=NULL;
		}
#endif
...
static int __init akm09911_init(void)
{
	...
	if (!hw)
	{
		MAGN_ERR("akm09911 get dts info fail\n");
	}

	if (platform_driver_register(&mag_driver) != 0)
	{
		MAGN_ERR("platform register mage driver fail\n");
	}
