...
static int mag_real_driver_init(void)
{
	...
	int err = 0;
#ifdef CONFIG_HQ_HWINFO
	mag_hwinfo = hwinfo_create("msensor");
	if(!mag_hwinfo)
		MAG_LOG("hwinfo create fail\r\n");
#endif
	...
	for (i = 0; i < MAX_CHOOSE_G_NUM; i++) {
		...
		if (0 != msensor_init_list[i]) {
			...
			if (0 == err) {
#ifdef CONFIG_HQ_HWINFO
				hwinfo_set_devinfo(mag_hwinfo,"%s",msensor_init_list[i]->name);
#endif
...
#ifdef CONFIG_HQ_HWINFO
	if(hwinfo_register(mag_hwinfo))
		MAG_LOG("hwinfo register fail\r\n");
#endif
