...
static struct dsm_dev dsm_mt_pmu = {
       .name           = "dsm_pmu",
       .fops           = NULL,
       .buff_size      = DSM_SENSOR_BUF_MAX,
};
 
static struct dsm_client *mt_pmu_dclient = NULL;
#define PMU_LDO1_VOL_CONFIG    1800000
#define PMU_LDO2_VOL_CONFIG    2800000
#define PMU_LDO3_VOL_CONFIG    2800000
#define PMU_LDO4_VOL_CONFIG    2800000
#define PMU_LDO5_VOL_CONFIG    2800000
#define PMU_LDO6_VOL_CONFIG    2800000
#define PMU_LDO7_VOL_CONFIG    3300000
#define PMU_LDO8_VOL_CONFIG    3300000
#define PMU_LDO9_VOL_CONFIG    3300000
#define PMU_LDO10_VOL_CONFIG   0
#define PMU_LDO11_VOL_CONFIG   1800000
#define PMU_LDO12_VOL_CONFIG   1800000
#define PMU_LDO13_VOL_CONFIG   3300000
#define PMU_LDO14_VOL_CONFIG   3000000
#define PMU_LDO15_VOL_CONFIG   1800000
#define PMU_LDO16_VOL_CONFIG   3000000
#define PMU_LDO17_VOL_CONFIG   2800000
#define PMU_LDO18_VOL_CONFIG   2800000
#define PMU_LDO19_VOL_CONFIG   1200000
#define PMU_LDO20_VOL_CONFIG   2800000
#define PMU_LDO21_VOL_CONFIG   1200000
#define PMU_LDO22_VOL_CONFIG   1800000
#define PMU_LDO23_VOL_CONFIG   1800000
#define PMU_LDO24_VOL_CONFIG   1800000
#define PMU_LDO25_VOL_CONFIG   1800000
#define PMU_LDO26_VOL_CONFIG   1800000
#define PMU_LDO27_VOL_CONFIG   0
#define PMU_LDO28_VOL_CONFIG   1200000
#endif
/*****************************************************************************
...
#endif
#define META_MODE_LONG_PWRKEY_PRESS_TIME         (5000*1000000)
...
#ifdef CONFIG_HUAWEI_DSM
int dms_pmu_monitor_report_err(int voltage, int errno)
{
       int size = 0;

       if(dsm_client_ocuppy(mt_pmu_dclient)){
               /* buffer is busy */
       ...
       switch (errno)
       {
               case DSM_MT_PA_TZ_ERROR_NO:
                       dsm_client_record(mt_pmu_dclient,"[mtkts_btsmdpa_get_hw_temp] T_btsmdpa > 70\n");
                       break;
               case DSM_MT_CPU_TZ_ERROR_NO:
                       dsm_client_record(mt_pmu_dclient,"[mtkts_bts_get_hw_temp] T_AP > 70\n");
                       break;
               case DSM_MT_LDO1_VOL_ERROR_NO:
                       dsm_client_record(mt_pmu_dclient,"ldo1 name:%s, voltage:%duv\n", mtk_ldos[0], voltage);
                       break;
               case DSM_MT_LDO2_VOL_ERROR_NO:
                       dsm_client_record(mt_pmu_dclient,"ldo1 name:%s, voltage:%duv\n", mtk_ldos[1], voltage);
                       break;
               case DSM_MT_LDO3_VOL_ERROR_NO:
                       dsm_client_record(mt_pmu_dclient,"ldo1 name:%s, voltage:%duv\n", mtk_ldos[2], voltage);
                       break;

               case DSM_MT_LDO5_VOL_ERROR_NO:
                       dsm_client_record(mt_pmu_dclient,"ldo1 name:%s, voltage:%duv\n", mtk_ldos[4], voltage);
                       break;
               case DSM_MT_LDO6_VOL_ERROR_NO:
                       dsm_client_record(mt_pmu_dclient,"ldo1 name:%s, voltage:%duv\n", mtk_ldos[5], voltage);
                       break;
               case DSM_MT_LDO7_VOL_ERROR_NO:
                       dsm_client_record(mt_pmu_dclient,"ldo1 name:%s, voltage:%duv\n", mtk_ldos[6], voltage);
                       break;
               case DSM_MT_LDO8_VOL_ERROR_NO:
                       dsm_client_record(mt_pmu_dclient,"ldo1 name:%s, voltage:%duv\n", mtk_ldos[7], voltage);
                       break;
               case DSM_MT_LDO9_VOL_ERROR_NO:
                       dsm_client_record(mt_pmu_dclient,"ldo1 name:%s, voltage:%duv\n", mtk_ldos[8], voltage);
                       break;

               case DSM_MT_LDO11_VOL_ERROR_NO:
                       dsm_client_record(mt_pmu_dclient,"ldo1 name:%s, voltage:%duv\n", mtk_ldos[10], voltage);
                       break;
               case DSM_MT_LDO12_VOL_ERROR_NO:
                       dsm_client_record(mt_pmu_dclient,"ldo1 name:%s, voltage:%duv\n", mtk_ldos[11], voltage);
                       break;
               case DSM_MT_LDO13_VOL_ERROR_NO:
                       dsm_client_record(mt_pmu_dclient,"ldo1 name:%s, voltage:%duv\n", mtk_ldos[12], voltage);
                       break;
               case DSM_MT_LDO14_VOL_ERROR_NO:
                       dsm_client_record(mt_pmu_dclient,"ldo1 name:%s, voltage:%duv\n", mtk_ldos[13], voltage);
                       break;
               case DSM_MT_LDO15_VOL_ERROR_NO:
                       dsm_client_record(mt_pmu_dclient,"ldo1 name:%s, voltage:%duv\n", mtk_ldos[14], voltage);
                       break;

               case DSM_MT_LDO17_VOL_ERROR_NO:
                       dsm_client_record(mt_pmu_dclient,"ldo1 name:%s, voltage:%duv\n", mtk_ldos[16], voltage);
                       break;
               case DSM_MT_LDO18_VOL_ERROR_NO:
                       dsm_client_record(mt_pmu_dclient,"ldo1 name:%s, voltage:%duv\n", mtk_ldos[17], voltage);
                       break;
               case DSM_MT_LDO19_VOL_ERROR_NO:
                       dsm_client_record(mt_pmu_dclient,"ldo1 name:%s, voltage:%duv\n", mtk_ldos[18], voltage);
                       break;
               case DSM_MT_LDO20_VOL_ERROR_NO:
                       dsm_client_record(mt_pmu_dclient,"ldo1 name:%s, voltage:%duv\n", mtk_ldos[19], voltage);
                       break;
               case DSM_MT_LDO21_VOL_ERROR_NO:
                       dsm_client_record(mt_pmu_dclient,"ldo1 name:%s, voltage:%duv\n", mtk_ldos[20], voltage);
                       break;
               case DSM_MT_LDO22_VOL_ERROR_NO:
                       dsm_client_record(mt_pmu_dclient,"ldo1 name:%s, voltage:%duv\n", mtk_ldos[21], voltage);
                       break;

               case DSM_MT_LDO24_VOL_ERROR_NO:
                       dsm_client_record(mt_pmu_dclient,"ldo1 name:%s, voltage:%duv\n", mtk_ldos[23], voltage);
                       break;
			   case DSM_MT_LDO25_VOL_ERROR_NO:
                       dsm_client_record(mt_pmu_dclient,"ldo1 name:%s, voltage:%duv\n", mtk_ldos[24], voltage);
                       break;
               case DSM_MT_LDO26_VOL_ERROR_NO:
                       dsm_client_record(mt_pmu_dclient,"ldo1 name:%s, voltage:%duv\n", mtk_ldos[25], voltage);
                       break;

               case DSM_MT_LDO28_VOL_ERROR_NO:
                       dsm_client_record(mt_pmu_dclient,"ldo1 name:%s, voltage:%duv\n", mtk_ldos[27], voltage);
                       break;
               default:
                       break;
       ...
       dsm_client_notify(mt_pmu_dclient, errno);

       return size;
}

int mt_ldo_update_thread_dsm(void *x)
{
       int i;
       int en = 0;
       int voltage_reg = 0;
       ...
       while (1) {
               msleep(60 * 1000); // sleep 1min wait device_monitor thread up.
               for (i = 0; i < 27; i++) {
                       ...
                       if(en) {
                               if(i == 0 && (abs(voltage -PMU_LDO1_VOL_CONFIG) > 200000))
                                       dms_pmu_monitor_report_err(voltage, DSM_MT_LDO1_VOL_ERROR_NO);
                               if(i == 1 && (abs(voltage -PMU_LDO2_VOL_CONFIG) > 200000))
                                       dms_pmu_monitor_report_err(voltage, DSM_MT_LDO2_VOL_ERROR_NO);
                               if(i == 2 && (abs(voltage -PMU_LDO3_VOL_CONFIG) > 200000))
                                       dms_pmu_monitor_report_err(voltage, DSM_MT_LDO3_VOL_ERROR_NO);

                               if(i == 4 && (abs(voltage -PMU_LDO5_VOL_CONFIG) > 200000))
                                       dms_pmu_monitor_report_err(voltage, DSM_MT_LDO5_VOL_ERROR_NO);
                               if(i == 5 && (abs(voltage -PMU_LDO6_VOL_CONFIG) > 200000))
                                       dms_pmu_monitor_report_err(voltage, DSM_MT_LDO6_VOL_ERROR_NO);
                               if(i == 6 && (abs(voltage -PMU_LDO7_VOL_CONFIG) > 200000))
                                       dms_pmu_monitor_report_err(voltage, DSM_MT_LDO7_VOL_ERROR_NO);
                               if(i == 7 && (abs(voltage -PMU_LDO8_VOL_CONFIG) > 200000))
                                       dms_pmu_monitor_report_err(voltage, DSM_MT_LDO8_VOL_ERROR_NO);
                               if(i == 8 && (abs(voltage -PMU_LDO9_VOL_CONFIG) > 200000))
                                       dms_pmu_monitor_report_err(voltage, DSM_MT_LDO9_VOL_ERROR_NO);

                               if(i == 10 && (abs(voltage -PMU_LDO11_VOL_CONFIG) > 200000))
                                       dms_pmu_monitor_report_err(voltage, DSM_MT_LDO11_VOL_ERROR_NO);
                               if(i == 11 && (abs(voltage -PMU_LDO12_VOL_CONFIG) > 200000))
                                       dms_pmu_monitor_report_err(voltage, DSM_MT_LDO12_VOL_ERROR_NO);
							   if(i == 12 && (abs(voltage -PMU_LDO13_VOL_CONFIG) > 200000))
                                       dms_pmu_monitor_report_err(voltage, DSM_MT_LDO13_VOL_ERROR_NO);
							   if(i == 13 && (abs(voltage -PMU_LDO14_VOL_CONFIG) > 200000))
                                       dms_pmu_monitor_report_err(voltage, DSM_MT_LDO14_VOL_ERROR_NO);
							   if(i == 14 && (abs(voltage -PMU_LDO15_VOL_CONFIG) > 200000))
                                       dms_pmu_monitor_report_err(voltage, DSM_MT_LDO15_VOL_ERROR_NO);

                               if(i == 16 && (abs(voltage -PMU_LDO17_VOL_CONFIG) > 200000))
                                       dms_pmu_monitor_report_err(voltage, DSM_MT_LDO17_VOL_ERROR_NO);
                               if(i == 17 && (abs(voltage -PMU_LDO18_VOL_CONFIG) > 200000))
                                       dms_pmu_monitor_report_err(voltage, DSM_MT_LDO18_VOL_ERROR_NO);
							   if(i == 18 && (abs(voltage -PMU_LDO19_VOL_CONFIG) > 200000))
                                       dms_pmu_monitor_report_err(voltage, DSM_MT_LDO19_VOL_ERROR_NO);
							   if(i == 19 && (abs(voltage -PMU_LDO20_VOL_CONFIG) > 200000))
                                       dms_pmu_monitor_report_err(voltage, DSM_MT_LDO20_VOL_ERROR_NO);
                               if(i == 20 && (abs(voltage -PMU_LDO21_VOL_CONFIG) > 200000))
                                       dms_pmu_monitor_report_err(voltage, DSM_MT_LDO21_VOL_ERROR_NO);
                               if(i == 21 && (abs(voltage -PMU_LDO22_VOL_CONFIG) > 200000))
                                       dms_pmu_monitor_report_err(voltage, DSM_MT_LDO22_VOL_ERROR_NO);

							   if(i == 23 && (abs(voltage -PMU_LDO24_VOL_CONFIG) > 200000))
                                       dms_pmu_monitor_report_err(voltage, DSM_MT_LDO24_VOL_ERROR_NO);
							   if(i == 24 && (abs(voltage -PMU_LDO25_VOL_CONFIG) > 200000))
                                       dms_pmu_monitor_report_err(voltage, DSM_MT_LDO25_VOL_ERROR_NO);
							   if(i == 25 && (abs(voltage -PMU_LDO26_VOL_CONFIG) > 200000))
                                       dms_pmu_monitor_report_err(voltage, DSM_MT_LDO26_VOL_ERROR_NO);

							   if(i == 27 && (abs(voltage -PMU_LDO28_VOL_CONFIG) > 200000))
                                       dms_pmu_monitor_report_err(voltage, DSM_MT_LDO28_VOL_ERROR_NO);
                       }
               }
       }
       return 0;
...
static int pmic_mt_cust_remove(struct platform_device *pdev)
{
	...
	#ifdef CONFIG_HUAWEI_DSM
       dsm_unregister_client(mt_pmu_dclient,&dsm_mt_pmu);
	#endif
	return 0;
}
...
#endif				/* End of #if !defined CONFIG_MTK_LEGACY */
#ifdef CONFIG_HUAWEI_DSM
       mt_pmu_dclient = dsm_register_client(&dsm_mt_pmu);
       if(!mt_pmu_dclient) {
               PMICLOG("%s: register dsm_dclient failed!!, line = %d\n", __func__,__LINE__);
		}
#endif
#ifdef CONFIG_HUAWEI_DSM
			kthread_run(mt_ldo_update_thread_dsm, NULL, "mt_ldo_update_thread_dsm");
#endif
}
...
#define META_LONG_PRESS_POWER_OFF
...
#endif
#if defined(META_LONG_PRESS_POWER_OFF)
	if (get_boot_mode() == META_BOOT && timer_pre != 0) {
		timer_pos = sched_clock();
		if (timer_pos - timer_pre >= META_MODE_LONG_PWRKEY_PRESS_TIME)
			long_pwrkey_press = true;
		PMICLOG
			("timer_pos = %ld, timer_pre = %ld, timer_pos-timer_pre = %ld, long_pwrkey_press = %d\r\n",
		 	timer_pos, timer_pre, timer_pos - timer_pre, long_pwrkey_press);
		if (long_pwrkey_press) {	/*500ms */
			PMICLOG
				("zcy Power Key Pressed during kernel power off charging, power off OS\r\n");
			kernel_power_off();
	}
}
#endif
...
void PMIC_INIT_SETTING_V1(void)
{
	...
	if (is_ext_buck_exist() == 1) {
		...
		ret = pmic_config_interface(0xA38, 0x1, 0x1, 10); // vibrator OCP
		ret = pmic_config_interface(0xA44, 0x1, 0x1, 1);
		ret = pmic_config_interface(0xA46, 0x0, 0x1, 14);
		ret = pmic_config_interface(0xA64, 0x3, 0x3, 4); // VCC volatge: 3.3V
        ...
        /*UOLV volatge modify:2.9v begin*/
		ret = pmic_config_interface(0xF6A,0x8,0x1F,0); // [4:4]: RG_EN_DRVSEL; Ricky
        /*UOLV volatge modify:2.9v end*/
        /*UOLV volatge modify:2.9v begin*/
                ret = pmic_config_interface(0xF6C,0x6,0x1F,0); // [4:4]: RG_EN_DRVSEL; Ricky
        /*UOLV volatge modify:2.9v end*/
		...
		ret = pmic_config_interface(0xA44, 0x1, 0x1, 1);
		ret = pmic_config_interface(0xA46, 0x0, 0x1, 14);
		ret = pmic_config_interface(0xA46, 0x1, 0x1, 0);
		ret = pmic_config_interface(0xA48,0x0,0x1,12);
		ret = pmic_config_interface(0xA64, 0x3, 0x3, 4);// VEMC volatge: 3.3V
		ret = pmic_config_interface(0xCBC, 0x1, 0x1, 8);
		ret = pmic_config_interface(0xCBC, 0x1, 0x1, 9);
		ret = pmic_config_interface(0xCC0, 0x24, 0xFFFF, 0);
		ret = pmic_config_interface(0xCC2, 0x14, 0xFF, 0);
