...
const unsigned int CS_VTH_20percent[] = {
	10240, 11520, 12800, 14080,
	15360, 16640, 17920, 19200,
	20480, 21760, 23040, 24320,
	25600, 26880, 28160, 29440,
	30720, 32000, 33280, 34560,
	35840, 37120, 38400, 39680,
	40960, 42240, 43520, 44800,
	46080, 47360, 48640, 49920
};
...
static unsigned int charging_hw_init(void *data)
{
	...
	mt_set_gpio_mode(BQ24196_EN_GPIO, GPIO_MODE_GPIO);
	mt_set_gpio_dir(BQ24196_EN_GPIO, GPIO_DIR_OUT);
	mt_set_gpio_out(BQ24196_EN_GPIO, GPIO_OUT_ZERO);

	bq24196_set_en_hiz(0x0);
	bq24196_set_vindpm(0xA); //VIN DPM check 4.6V ->4.68V
...
void bq24196_check_error_status(void)
{
	unsigned int ret;

	ret=bq24196_get_watchdog_fault();
	if(ret)
		battery_log(BAT_LOG_CRTI,"[ERROR]BQ24196 watchdog timer expiration\n");

	ret=bq24196_get_chrg_fault();
	if(ret==1)
		battery_log(BAT_LOG_CRTI,"[ERROR]BQ24196 power supply vbus ovp\n");
	else if(ret==2)
		battery_log(BAT_LOG_CRTI,"[ERROR]BQ24196 thermal shutdown\n");
	else if(ret==3)
		battery_log(BAT_LOG_CRTI,"[ERROR]BQ24196 charge safety time expiration\n");

	ret=bq24196_get_bat_fault();
	if(ret)
		battery_log(BAT_LOG_CRTI,"[ERROR]BQ24196 battery fault batovp\n");
}
#endif
...
static unsigned int charging_enable(void *data)
{
	...
	if (KAL_TRUE == enable) {
		...
		if (mt_usb_is_device()) {
#endif
			bq24196_set_watchdog(0x0); //disable WDT
...
static unsigned int charging_set_current(void *data)
{
	...
	if(current_value>=51200){
	battery_log(BAT_LOG_CRTI,"bq24196 charging_set_current current_value %d\n",current_value);
	bq24196_set_force_20pct(0);
	array_size = GETARRAYNUM(CS_VTH);
	...
	array_size = GETARRAYNUM(CS_VTH_20percent);
	set_chr_current = bmt_find_closest_level(CS_VTH_20percent, array_size, current_value);
	register_value = charging_parameter_to_value(CS_VTH_20percent, array_size, set_chr_current);
	bq24196_set_ichg(register_value);	
	}
	battery_log(BAT_LOG_CRTI,"bq24196 charging_set_current register_value %d\n",register_value);
	return status;
}
...
unsigned int bootmode=0;
	...
	battery_log(BAT_LOG_CRTI,"bq24196 charging_set_input_current array_size %d\n",array_size);
	set_chr_current = bmt_find_closest_level(INPUT_CS_VTH, array_size, current_value);
	register_value = charging_parameter_to_value(INPUT_CS_VTH, array_size, set_chr_current);
	bootmode = get_boot_mode();
		if ((bootmode == META_BOOT) || (bootmode == ADVMETA_BOOT))
			{
			bq24196_set_iinlim(0);
			battery_log(BAT_LOG_CRTI,"faj set_input_current=100mA\n");
			}
		else
