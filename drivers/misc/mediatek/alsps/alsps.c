...
#include "hwinfo.h"
struct hwinfo *alsps_hwinfo = NULL;
#endif
...
static int alsps_real_driver_init(void)
{
	...
	int err = 0;
#ifdef CONFIG_HQ_HWINFO
	alsps_hwinfo = hwinfo_create("alsps");
	if(!alsps_hwinfo)
		ALSPS_LOG("hwinfo create fail\r\n");
#endif
	...
	for (i = 0; i < MAX_CHOOSE_ALSPS_NUM; i++) {
		...
		if (0 != alsps_init_list[i]) {
			...
			if (0 == err) {
#ifdef CONFIG_HQ_HWINFO
				hwinfo_set_devinfo(alsps_hwinfo ,"%s", alsps_init_list[i]->name);
#endif
...
#ifdef CONFIG_HQ_HWINFO
	if(hwinfo_register(alsps_hwinfo))
		ALSPS_LOG("register hwinfo fail\r\n");
#endif
