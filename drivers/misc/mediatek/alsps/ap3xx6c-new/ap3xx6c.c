...
#define CLIENT_NAME_SENSOR_AP3426		"dsm_sensor_ap3426"
static int rgb_ap3426_debug_mask= 1;
static struct dsm_dev dsm_sensor_ap3426 = {
	.name 		= CLIENT_NAME_SENSOR_AP3426,		// dsm client name
	.fops 		= NULL,						      // options
	.buff_size 	= DSM_SENSOR_BUF_MAX,			// buffer size
};
#define AP3426_ERR(x...) do {\
    if (rgb_ap3426_debug_mask >=0) \
        printk(KERN_ERR x);\
    } while (0)

#define AP3426_INFO(x...) do {\
    if (rgb_ap3426_debug_mask >=1) \
        printk(KERN_ERR x);\
    } while (0)
#define AP3426_FLOW(x...) do {\
    if (rgb_ap3426_debug_mask >=2) \
        printk(KERN_ERR x);\
    } while (0)
static struct dsm_client *ap3426_sensor_dclient = NULL;
static int ap3426_dsm_report_err(int errno,int type)
{
	int size = 0;
	if (FACTORY_BOOT == get_boot_mode() ||(ap3426_sensor_dclient == NULL))
	return 0;
	if(dsm_client_ocuppy(ap3426_sensor_dclient)){
		/* buffer is busy */
		AP3426_ERR("%s: buffer is busy!, errno = %d\n", __func__,errno);
		return -EBUSY;
	}

	AP3426_INFO("dsm error, errno = %d \n", errno);
	if(type == 1){
			size = dsm_client_record(ap3426_sensor_dclient,"i2c_master_send or i2c_master_recv fail ,i2c erro type:%d\n",-EIO);
		}
	if(type == 2){
			size = dsm_client_record(ap3426_sensor_dclient,"CLIENT CANN'T EQUAL NULL ,i2c erro type:%d\n",-EIO);
		}
	if(type == 3){
			size = dsm_client_record(ap3426_sensor_dclient,"als_value is invalid!! ,i2c erro type:%d\n",-EIO);
		}
	...
	dsm_client_notify(ap3426_sensor_dclient, errno);
...
EXIT_ERR:
	...
	mutex_unlock(&ap3xx6_obj->lock);
#ifdef CONFIG_HUAWEI_DSM
     ap3426_dsm_report_err(DSM_LPSENSOR_I2C_ERROR_NO,1);
#endif
		return ret;
}
...
static int ap3xx6_get_ALSGain(struct i2c_client *client)
{
	int re_val = 0;

#ifdef AP3426
	re_val = ap3xx6_read_reg(client, 0x10,0xFF, 0x00);
	re_val = ((re_val&0x30)>>4);
#endif
...
static int ap3xx6_get_PSgain(struct i2c_client *client)
{
	int re_val = 0;
...
static int ap3xx6_init_client(struct i2c_client *client)
{
    ...
    ap3xx6_set_PSTtime(client, 0x06); //ps integrate time//0x08
    ap3xx6_set_PSgain(client, 0x01); //ps gain
	ap3xx6_set_PSpulse(client, 0x03);
    ap3xx6_set_meantime(client, 0x03);
//	ap3xx6_set_pcrosstalk(client,0x00); 
...
int ap3xx6_read_als(struct i2c_client *client, u16 *data)
{
	...
	if (client == NULL) {
 #ifdef CONFIG_HUAWEI_DSM
     ap3426_dsm_report_err(DSM_LPSENSOR_I2C_ERROR_NO,2);
 #endif
		APS_DBG("CLIENT CANN'T EQUAL NULL\n");
		return -1;
	}
...
#ifdef CONFIG_HUAWEI_DSM
     ap3426_dsm_report_err(DSM_LPSENSOR_I2C_ERROR_NO,3);
#endif
		goto EXIT_ERR;
	}
...
static int ap3xx6_Calibration_every_time(struct i2c_client *client)
{
	...
	int i = 0;
	u16 ui_ps_data = 0;
    u16 gi_offset_h=gi_ps_raw_data+20;
    u16 gi_offset_l=gi_ps_raw_data-20;
	/* struct i2c_client *client = (struct i2c_client*)file->private_data; */
	struct ap3xx6_priv *obj = i2c_get_clientdata(client);
//    int ps_cal = 0;
...
static void ap3xx6_eint_work(struct work_struct *work)
{
	...
	//	sensor_data.status = SENSOR_STATUS_ACCURACY_MEDIUM;
		...
		printk(KERN_ERR "ap3xx6: ap3xx6_eint_work ps value = %d \n",ps_value);
...
static ssize_t ap3xx6_enable_show(struct device_driver *ddri, char *buf)
{
	int ret =0;
	int count=0;
	...
	if(ps_enable_system)
		count += sprintf(buf+count, "AP3426 ps enable\n");
	else
		count += sprintf(buf+count, "AP3426 ps disable\n");
	return count;
}

static ssize_t ap3xx6_enable_store(struct device_driver *ddri, const char *buf, size_t count)
{
	int val=0,ret=0;
	...
	ap3xx6_enable_ps(ap3xx6_obj->client, val);
	return count;
}

static ssize_t ap3xx6_ps_gain_show(struct device_driver *ddri, char *buf)
{
	int ret_val =0;
	int count=0;
	...
	ret_val = ap3xx6_get_PSgain(ap3xx6_obj->client);
	count += snprintf(buf+count, PAGE_SIZE, "%u\n", ret_val);
	return count;
}

static ssize_t ap3xx6_ps_gain_store(struct device_driver *ddri, const char *buf, size_t count)
{
	int val=0,ret=0;
	...
	ap3xx6_set_PSgain(ap3xx6_obj->client, val);
	return count;
}

static ssize_t ap3xx6_als_gain_show(struct device_driver *ddri, char *buf)
{
	int ret_val =0;
	int count=0;
	...
	ret_val = ap3xx6_get_ALSGain(ap3xx6_obj->client);
	count += snprintf(buf+count, PAGE_SIZE, "%u\n", ret_val);
	return count;
}
static ssize_t ap3xx6_als_gain_store(struct device_driver *ddri, const char *buf, size_t count)
{
	int val=0,ret=0;
	...
	ap3xx6_set_ALSGain(ap3xx6_obj->client, val);
	return count;
}
...
static DRIVER_ATTR(ps_enable,  0660, ap3xx6_enable_show, ap3xx6_enable_store);
// by wuxiaolin 2016-07-15
static DRIVER_ATTR(ps_gain,  0660, ap3xx6_ps_gain_show, ap3xx6_ps_gain_store);
static DRIVER_ATTR(als_gain,  0660, ap3xx6_als_gain_show, ap3xx6_als_gain_store);
// end by wuxiaolin 2016-07-15
...
static long ap3xx6_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
	...
	case ALSPS_GET_PS_GAIN: // by wuxiaolin 2016-07-15
			ps_gain = ap3xx6_get_PSgain(obj->client);
	...
	case ALSPS_SET_PS_GAIN: // by wuxiaolin 2016-07-15
			if (copy_from_user(&ps_gain, ptr, sizeof(ps_gain))) {
                err = -EFAULT;
                goto err_out_ap3xx6_ioctl;
            }
			err = ap3xx6_set_PSgain(obj->client, ps_gain);
            if (err <= 0) {
                goto err_out_ap3xx6_ioctl;
            }
            break;
	...
	case ALSPS_GET_ALS_GAIN: // by wuxiaolin 2016-07-15
			als_gain = ap3xx6_get_ALSGain(obj->client);
	...
	case ALSPS_SET_ALS_GAIN: // by wuxiaolin 2016-07-15
			if (copy_from_user(&als_gain, ptr, sizeof(als_gain))) {
                err = -EFAULT;
                goto err_out_ap3xx6_ioctl;
            }
			err = ap3xx6_set_ALSGain(obj->client, als_gain);
            if (err <= 0) {
                goto err_out_ap3xx6_ioctl;
            }
            break;
...
//add by zym,2015-07-23 for ps calibration
	...
	case ALSPS_GET_CALIBRATION_DATA:
           ...
           if(!test_bit(CMC_BIT_PS, &obj->enable)){
			...
			alsps_calibrate.z = alsps_get_corss_talk(obj, 10); // ps cross talk
...
// *value = (obj->als)*1136/1000;  //modify by yifaguo 2016.07.29
        *value = (obj->als)*1105/1000;  //modify by yifaguo 2016.08.02
...
// *value = (obj->als)*1000/1538;     //kevin20150710
		*value = (obj->als)*694/1000;  //modify by kevin 2016.11.15
        *status = SENSOR_STATUS_ACCURACY_MEDIUM;
...
#ifdef CONFIG_HUAWEI_DSM
	ap3426_sensor_dclient = dsm_register_client(&dsm_sensor_ap3426);
	if(!ap3426_sensor_dclient)
		AP3426_ERR("%s: register dsm_dclient failed!!, line = %d\n", __func__,__LINE__);
#endif
...
static int ap3xx6_i2c_remove(struct i2c_client *client)
{
	...
	if(ap3426_sensor_dclient)
	{
		dsm_unregister_client(ap3426_sensor_dclient,&dsm_sensor_ap3426);
		ap3426_sensor_dclient=NULL;
	}
#endif
...
static int ap3xx6_local_init(void)
{
	...
	APS_ERR("ap3xx6_local_init start...\n");
...
{
	const char *name = "mediatek,AP3xx6c";
	APS_FUN();
	hw = get_alsps_dts_func(name, hw);
	if (!hw)
		APS_ERR("%s get dts info fail\n",name);
	else
		APS_ERR("%s get dts info succeed\n",name);
