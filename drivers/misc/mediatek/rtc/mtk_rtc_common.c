...
void rtc_mark_erecovery(void)
{
	unsigned long flags;
	rtc_xinfo("rtc_mark_erecovery\n");
	spin_lock_irqsave(&rtc_lock, flags);
	hal_rtc_set_spare_register(RTC_FAC_RESET, 0x3);
	spin_unlock_irqrestore(&rtc_lock, flags);
}

#ifdef CONFIG_HUAWEI_DM_VERITY_SUPPORT
void rtc_mark_logging(void)
{
	unsigned long flags;
	rtc_xinfo("rtc_mark_logging\n");
	spin_lock_irqsave(&rtc_lock, flags);
