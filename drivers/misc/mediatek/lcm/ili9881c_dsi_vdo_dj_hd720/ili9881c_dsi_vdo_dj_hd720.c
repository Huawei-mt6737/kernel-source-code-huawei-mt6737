...
static void lcm_get_params(LCM_PARAMS *params)
{
	...
	// Bit rate calculation
		...
		params->dsi.PLL_CLOCK=214;//208//270		
...
static void lcm_init_power(void)
{
	...
	LCD_DEBUG("[ili9881c-dj]: %s, line%d,	ret=%d\n", __func__, __LINE__, ret);
	MDELAY(2);
...
static unsigned int lcm_compare_id(void)
{
	...
	LCD_DEBUG("[ili9881c-dj]: %s, line%d, id0=%x\n", __func__, __LINE__, id[0]);
	LCD_DEBUG("[ili9881c-dj]: %s, line%d, id1=%x\n", __func__, __LINE__, id[1]);
	LCD_DEBUG("[ili9881c-dj]: %s, line%d, id2=%x\n", __func__, __LINE__, id[2]);

	if(id[0] == 0x98 && id[1] == 0x81)// && id[2] == 0x1C)
		return 1;

	LCD_DEBUG("[ili9881c-dj]: read id fail !\n");
	return 0;
}

LCM_DRIVER ili9881c_dsi_vdo_dj_hd720_lcm_drv = 
{
	.name		= "ili9881c_dsi_vdo_dj_hd720",
