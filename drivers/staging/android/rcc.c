 ...
 *    cpu load in percent.
 */
static int hwget_cpu_load(struct rcc_module *rcc, bool need_delay)
{
	int cpu_load;
...
static int rcc_thread(void *unused)
{
	...
	while (true) {
		...
		ret = rcc_thread_wait(rcc, RCC_IDLE_SLOW);
		if (kthread_should_stop())
			goto out;
		/* force update cpu load stat. */
		hwget_cpu_load(rcc, true);
...
static int rcc_thread_start(struct rcc_module *rcc)
{
	...
	task = kthread_run(rcc_thread, NULL, "rcc");
	if (IS_ERR(task)) {
				      ...
				      const char *buf, size_t len)
{
	...
	if (!size)
		return -EINVAL;
	rcc->full_clean_file_pages = (size >> PAGE_SHIFT);
	return len;
}
			     ...
			     struct kobj_attribute *attr, char *buf)
{
	struct rcc_module *rcc = &rcc_module;
	return scnprintf(buf, PAGE_SIZE,
		"clean pages: full=%d, normal=%d; wake count: %d, time=%d\n",
