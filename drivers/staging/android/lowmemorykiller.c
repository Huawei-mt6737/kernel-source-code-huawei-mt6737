...
static unsigned long lowmem_scan(struct shrinker *s, struct shrink_control *sc)
{
	...
	struct task_struct *selected_1 = NULL;
	struct task_struct *selected_2 = NULL;
	unsigned long rem = 0;
	int tasksize;
	int i;
	...
	int selected_tasksize_1 = 0;
	int selected_tasksize_2 = 0;
	short selected_oom_score_adj_1;
	...
	*/
	...
	if (vm_swap_full()) {
		other_free >>= 1;
		lowmem_print(3, "Halved other_free %d\n", other_free);
	}
#endif
#ifdef CONFIG_SWAP
	...
	selected_oom_score_adj = min_score_adj;
       selected_oom_score_adj_1 = min_score_adj;
...
log_again:
		...
		*/
		...
		if (tasksize <= 0)
			continue;
		if (selected_1) {
			if (oom_score_adj < selected_oom_score_adj_1)
				continue;
		}
		if ((selected_1)&&(oom_score_adj > lowmem_adj[2])) {
			if (tasksize > selected_tasksize_1){
				selected_tasksize_1 = tasksize;
				selected_1 = p;
			} else {

				if (selected_2) {
					if (tasksize > selected_tasksize_2){
						selected_tasksize_2 = tasksize;
						selected_2 = p;
					}
				} else {
					selected_2 = p;
	...
	if (selected) {
		...
		*/
		...
		if((selected_1)&&(selected_1 != selected)) {
			set_tsk_thread_flag(selected_1, TIF_MEMDIE);
			send_sig(SIGKILL, selected_1, 0);
			rem += selected_tasksize_1;

		} else if((selected_2)&&(selected_2 != selected)) {
			set_tsk_thread_flag(selected_2, TIF_MEMDIE);
			send_sig(SIGKILL, selected_2, 0);
			rem += selected_tasksize_2;
		}
