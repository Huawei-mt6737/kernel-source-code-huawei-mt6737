	...
	client = dsm_find_client(client_name);
		...
		if (client && !strncasecmp("sdcard_vold", client_name, size) && (!dsm_client_ocuppy(client))) {
			...
			if (*ptr == '\n') {
				...
				if (sscanf(err_string, "%d", &err) != 1) {
					DSM_LOG_ERR("%s: sscanf failed\n", __func__);
				}
...
printk(KERN_ERR "try bind client %s\n", buff);
				client = dsm_find_client(buff);
				if (client) {
printk(KERN_ERR "dsm_ioctl 1\n");
					dsm_bind_client(client);
					file->private_data = (void *)client;
				} else {
printk(KERN_ERR "dsm_ioctl 2\n");
					ret = -ENXIO;
				}
			}
			break;
	...
	case DSM_IOCTL_FORCE_DUMP:
			if (copy_from_user(buff, argp, UINT_BUF_MAX)) {
				//DSM_LOG_ERR("copy from user failed\n");
printk(KERN_ERR "DSM_IOCTL_FORCE_DUMP copy from user failed %s\n", buff);
				ret = -EFAULT;
			} else {
				if (client && client->cops && client->cops->dump_func) {
					if (!dsm_client_ocuppy(client)) {
						...
						set_bit(CBUFF_READY_BIT, &client->buff_flag);
printk(KERN_ERR "2 DSM_IOCTL_FORCE_DUMP copy from user failed %s\n", buff);
					} else {
printk(KERN_ERR "3 DSM_IOCTL_FORCE_DUMP copy from user failed %s\n", client->client_name);
						//DSM_LOG_INFO("client %s's buff ocuppy failed\n", client->client_name);
						ret = -EBUSY;
					}
				} else {
printk(KERN_ERR "4 DSM_IOCTL_FORCE_DUMP copy from user failed %s\n", buff);
