		...
		bool hw_busy_detect, struct request *req, int *gen_err)
{
	...
	do {
		...
		if (mmc_card_sd(card)){
			 ...
			 */
			//pr_err("%s: [SD_HEALTH_DETECT1]card_busy_detect,mmc_card_sd(card)\n",req->rq_disk->disk_name);
			if (time_after(jiffies, sd_timeout)) {
				pr_err("%s:[SD_HEALTH_DETECT1] SD card stuck in programming state!%s %s\n",mmc_hostname(card->host),req->rq_disk->disk_name, __func__);
				//kobject_uevent(&card->dev.kobj,KOBJ_SD_RW_ERROR);
				mmc_diag_sd_health_status(card, MMC_BLK_STUCK_IN_PRG_ERR);
...
static int mmc_blk_issue_rw_rq(struct mmc_queue *mq, struct request *rqc)
{
	...
	{
		//pr_err(" [SD_HEALTH_DETECT2]mmc_blk_issue_rw_rq,strcmp(current->comm,mmcqd/1)\n");
		mmc_trigger_ro_check(card,rqc,md->disk,md->read_only);
		time2 = sched_clock();
		rq_byte1 = mmc_calculate_ioworkload_and_rwspeed(card,time2,rqc,md->disk);
	}
#endif
...
/*/proc/stat end*/
	...
	do {
		...
		if(mmc_card_sd(card))
		{
			//pr_err("[SD_HEALTH_DETECT3]mmc_blk_issue_rw_rq,mmc_get_rw_status\n");
			mmc_diag_sd_health_status(card,mmc_get_rw_status(status));//
	...
	if(!strcmp(current->comm,"mmcqd/1"))
	{
		//pr_err("[SD_HEALTH_DETECT4]mmc_blk_issue_rw_rq,mmc_calculate_rw_size\n");
		mmc_calculate_rw_size(time2,rq_byte1,rqc);
	}
#endif
	return 1;
