...
static unsigned char CTPM_FW[] = {
	 #include  "HQ_AL828_Y53_Truly0x5a_V02_D01_20160708_app.i"
};

static unsigned char CTPM_FW_VENDOR_1[] = {
       #include "HQ_AL828_Y53_Truly0x5a_Ver18_20170413_app.i"
};

static unsigned char CTPM_FW_VENDOR_2[] = {
	#include "HQ_AL828_Y53_Booyi0xa6_Ver0a_20170306_app.i"
};

char *tp_firmware_ft="";//add by yifaguo for hwinfo 2016.07.14

char *tp_devinfo_ft = "";
int ft_firmware_version = 0;
...
int fts_ctpm_get_i_file_ver_auto(u8 vendor_id)
{
	...
	{
		ui_sz = sizeof(CTPM_FW_VENDOR_1);

		if(fts_updateinfo_curr.CHIP_ID==0x36  || fts_updateinfo_curr.CHIP_ID==0x86 || fts_updateinfo_curr.CHIP_ID==0x64)
		      return CTPM_FW_VENDOR_1[0x10a];
		else if(fts_updateinfo_curr.CHIP_ID==0x58)
		      return CTPM_FW_VENDOR_1[0x1D0A];
		else
			return CTPM_FW_VENDOR_1[ui_sz - 2];
	}
	else if(vendor_id == FTS_Vendor_2_ID) 
	{
		ui_sz = sizeof(CTPM_FW_VENDOR_2);
		
		if(fts_updateinfo_curr.CHIP_ID==0x36  || fts_updateinfo_curr.CHIP_ID==0x86 || fts_updateinfo_curr.CHIP_ID==0x64)
			return CTPM_FW_VENDOR_2[0x10a];
		else if(fts_updateinfo_curr.CHIP_ID==0x58)
			return CTPM_FW_VENDOR_2[0x1D0A];
		else
			return CTPM_FW_VENDOR_2[ui_sz - 2];
	}
...
int fts_ctpm_fw_upgrade_with_i_file_auto(struct i2c_client *client,u8 vendor_id)
{
	...
	if(vendor_id == FTS_Vendor_1_ID)
	{
		fw_len = sizeof(CTPM_FW_VENDOR_1);
		pbt_buf = CTPM_FW_VENDOR_1;
	}
	else if(vendor_id == FTS_Vendor_2_ID)
	{
		fw_len = sizeof(CTPM_FW_VENDOR_2);
		pbt_buf = CTPM_FW_VENDOR_2;
	}
	...
	*/
	...
	else if ((fts_updateinfo_curr.CHIP_ID==0x0E))
	{
		...
		//pbt_buf = CTPM_FW;
		i_ret = fts_3x07_ctpm_fw_upgrade(client, pbt_buf, fw_len);
		if (i_ret != 0)
			dev_err(&client->dev, "%s:upgrade failed. err.\n",__func__);
	}
	return i_ret;
...
static unsigned char ft5x46_ctpm_VidFWid_get_from_boot(  struct i2c_client *client )
{
	...
	{
		msleep(100);
		printk("[FTS]%s Step 1:Reset  CTPM\n",__func__);
		/*********Step 1:Reset  CTPM *****/
		...
		/*********Step 2:Enter upgrade mode *****/
		printk("[FTS]%s Step 2:Enter upgrade mode \n",__func__);
		auc_i2c_write_buf[0] = FTS_UPGRADE_55;
		fts_i2c_write(client, auc_i2c_write_buf, 1);
		msleep(5);
		auc_i2c_write_buf[0] = FTS_UPGRADE_AA;
		...
		printk("[FTS]%s Step 3: CTPM ID,ID1 = 0x%x,ID2 = 0x%x\n",__func__,reg_val[0], reg_val[1]);
		if (reg_val[0] == fts_updateinfo_curr.upgrade_id_1
			&& reg_val[1] != 0) {
	...
	printk("[FTS]%s Step 7: reset the new FW\n",__func__);
	auc_i2c_write_buf[0] = 0x07;
	fts_i2c_write(client, auc_i2c_write_buf, 1);
	msleep(200);	/*make sure CTP startup normally */
	return vid;     
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	printk("[FTS] uc_tp_fm_ver = 0x%x, uc_tp_vendor_id = 0x%x\n",uc_tp_fm_ver, uc_tp_vendor_id);
        ...
        if(FTS_Vendor_1_ID == uc_tp_vendor_id) 
        {
            tp_devinfo_ft = "FT3327,TRULY";
        } else if (FTS_Vendor_2_ID == uc_tp_vendor_id) {
            tp_devinfo_ft = "FT3427,BOOYI";
        }
	...
	//if(1)
	{
		...
		{
		#ifdef CONFIG_HUAWEI_DSM
			tpd_ft3x27_dsm_report_err(DSM_TP_FWUPDATE_ERROR_NO, i_ret);
		#endif
    ...
    if (!tp_firmware_kbuf) {
        printk("ftxx kmalloc buffer error");
        return -ENOMEM;
    }

    tp_firmware_ft = tp_firmware_kbuf;
    sprintf(tp_firmware_ft,"version:%2d",uc_tp_fm_ver);//if upgrade display new version
	ft_firmware_version = uc_tp_fm_ver;
	ctp_hwinfo_flag = 1;
	return 0;
}
