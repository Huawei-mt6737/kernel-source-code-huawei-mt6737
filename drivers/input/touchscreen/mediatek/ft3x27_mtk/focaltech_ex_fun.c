...
extern struct dsm_client *tpd_dclient;
extern int tpd_ft3x27_dsm_report_err(int errno,int type);
static ssize_t fts_dsmtest_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
	if (tpd_dclient != NULL)
	{
...
#ifdef CONFIG_HUAWEI_DSM
static DEVICE_ATTR(ftsdsmtest, S_IRUGO|S_IWUSR, NULL, fts_dsmtest_store);
#endif
