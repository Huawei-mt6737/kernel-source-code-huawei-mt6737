...
#include <mt_boot.h>
#define CLIENT_NAME_TOUCHPANEL		"dsm_tpd_ft3x27"
...
int tpd_ft3x27_dsm_report_err(int errno,int type)
{
	int size = 0;
...
//add by yifaguo for hwinfo 2016.07.14
extern char *tp_devinfo_ft;
extern int ft_firmware_version;
//extern int ft_wakeup_gesture;//ft_wakeup_gesture
extern unsigned short coordinate_x[255];
extern unsigned short coordinate_y[255];
static int tpd_halt = 0;
static int ft3327_sensitivity = 0;
static int ft3327_sensitivity_flag = -1;
...
static void tpd_usb_plugin(int plugin)
{
	...
	ret = fts_write_reg(i2c_client,0x8B,b_usb_plugin);
	if ( ret < 0 )
	{
...
static void charger_switch_process(void)
{
	int chr_status = 0;
	int ret = 0;
	...
	if (chr_status!=b_usb_plugin)
	{
		tpd_usb_plugin(chr_status);
	}
}
...
#ifdef FTS_SENSITIVITY_SWITCH
int tpd_state = 0;
int fts_is_probe = 0;

void fts_sensitivity_state_switch(int value)
{
	int ret = -1;
	tpd_state = value;
	...
	printk("[fts] sensitivity switch: %s %d.\n",__func__,tpd_state);
	ret = fts_write_reg(i2c_client,0xC8,tpd_state);
	if ( ret < 0 )
	{
		printk("[fts] sensitivity switch write err: %s %d.\n",__func__,tpd_state);
	}
}

//EXPORT_SYMBOL(fts_sensitivity_state_switch);
#endif
#ifdef TIMER_DEBUG
...
static void tpd_down(int x, int y, int p, int id)
{
...
#else
	{
		...
		printk(KERN_ERR "%s x:%d y:%d p:%d, device:%s, firmware: %d\n", __func__, x, y, p,tp_devinfo_ft,ft_firmware_version);
...
#ifdef CONFIG_HUAWEI_DSM
	if(ret < 0)
		tpd_ft3x27_dsm_report_err(DSM_TP_I2C_RW_ERROR_NO, ret);
#endif
	return ret;	
}
int fts_i2c_write(struct i2c_client *client, char *writebuf, int writelen)
{
	int i = 0;
	int ret = 1;
	printk("[FTS]CONFIG_MTK_I2C_EXTENSION,fts_i2c_write\n");
       mutex_lock(&i2c_rw_access);
...
int fts_write_reg(struct i2c_client *client, u8 regaddr, u8 regvalue)
{
	...
	ret = fts_i2c_write(client, buf, sizeof(buf));

#ifdef CONFIG_HUAWEI_DSM
	if(ret < 0)
		tpd_ft3x27_dsm_report_err(DSM_TP_I2C_RW_ERROR_NO, 1);
#endif
...
static int tpd_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	#endif
	#ifdef CONFIG_MTK_I2C_EXTENSION
		printk("[FTS]CONFIG_MTK_I2C_EXTENSION define\n");
	#else
		printk("[FTS]CONFIG_MTK_I2C_EXTENSION mo define\n");
	#endif
#ifdef CONFIG_FT_AUTO_UPGRADE_SUPPORT
...
reset_proc:
	...
	if(err< 0 || data!=0)// reg0 data running state is 0; other state is not 0
	{
		...
		printk("wangshouping I2C transfer error, line: %d\n", __LINE__);
...
//sprintf(g_tpd_module_name,"duntaictp");//wangshouping for ctp moudule
//g_hq_ctp_module_fw_version = ver;//wangshouping  for ctp version
	...
	if(ctp_is_probe == 0)
	{
		 fts_write_reg(i2c_client, 0x8B,  b_usb_plugin);
		ctp_is_probe =1;
	}
#endif
#ifdef FTS_SENSITIVITY_SWITCH
	if(fts_is_probe == 0)
	{
		 fts_write_reg(i2c_client, 0xC8,  tpd_state);
		fts_is_probe =1;
	}
#endif
	return 0;
...
static int tpd_local_init(void)
{
	...
	if (retval != 0) {
		printk("wangshouping Failed to set reg-vgp6 voltage: %d\n", retval);
		return -1;
	}
#endif
...
#endif
#ifdef FTS_SENSITIVITY_SWITCH
	fts_sensitivity_state_switch(tpd_state);
#endif
   	tpd_halt = 0;
	printk("tpd resume end\n");
}
...
static ssize_t ft3327_sensitivity_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", ft3327_sensitivity_flag);
}

static ssize_t ft3327_sensitivity_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	int ret = 0;
	if(!strcmp(buf,"0\n")){
		ft3327_sensitivity = 0;
		TPD_DMESG("mtk-tpd:ft3327_sensitivity is %d\n",ft3327_sensitivity);
	}
	else if(!strcmp(buf,"1\n")){
		ft3327_sensitivity = 1;
		TPD_DMESG("mtk-tpd:ft3327_sensitivity is %d\n",ft3327_sensitivity);
	}else{
		TPD_DMESG("mtk-tpd:ft3327_sensitivity is invalid %s\n",buf);
		ft3327_sensitivity_flag = -1;
		return size;
	}

	ft3327_sensitivity_flag = ft3327_sensitivity;
	fts_sensitivity_state_switch(ft3327_sensitivity_flag);

#if 0
	//to do flash firmware here
	ret = gtp_init_panel(i2c_client_point);
	if (ret < 0) {
		ft3327_sensitivity_flag = -1;
		TPD_DMESG("ft3327 send config failed.\n");
	}else{
		ft3327_sensitivity_flag = ft3327_sensitivity;
		TPD_DMESG("ft3327 send config success.\n");
	}
#endif
...
static DEVICE_ATTR(ctp_sensitivity, 0664, ft3327_sensitivity_show, ft3327_sensitivity_store);

static struct device_attribute *ft3327_attrs[] = {
	&dev_attr_ctp_sensitivity,
};

static struct tpd_driver_t tpd_device_driver = {
	.tpd_device_name = "FT3327",
	.tpd_local_init = tpd_local_init,
	.suspend = tpd_suspend,
	.resume = tpd_resume,
	.attrs = {
		.attr = ft3327_attrs,
		.num  = ARRAY_SIZE(ft3327_attrs),
	},
};
