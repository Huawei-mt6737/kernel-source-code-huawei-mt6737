...
static int tpd_info_flag = -1;
static int tpd_flag;
int tpd_halt = 0;
static int tpd_eint_mode = 1;
static struct task_struct *thread;
static int tpd_polling_time = 50;
static int gt615_sensitivity = 0;
static int gt615_sensitivity_flag = -1;
#ifdef CONFIG_HUAWEI_DSM
#include <mt_boot.h>
#define CLIENT_NAME_TOUCHPANEL		"dsm_tpd_gt9xx"
...
int tpd_gt9xx_dsm_report_err(int errno,int type)
{
	int size = 0;
...
static const u16 touch_key_array[] = TPD_KEYS; //closed by yifaguo 2016.06.28
...
#pragma pack()
char *tp_devinfo_gt = NULL;
char *tp_firmware_gt = "";//add by yifaguo for hwinfo 2016.07.14
int gt_cfg_version = 0;
int gt_ic_cfg_version = 0;
int gt_firmware_ver_low = 0;
int gt_firmware_ver_high = 0;
extern int ctp_hwinfo_flag;
...
#ifdef GTP_MUTI_FW_COMPATIBLE
u8 global_sensor_id = 0xff;
#endif
...
static const struct i2c_device_id tpd_i2c_id[] = {{"gt615", 0}, {} };
static unsigned short force[] = {0, 0xBA, I2C_CLIENT_END, I2C_CLIENT_END};
static const unsigned short *const forces[] = { force, NULL };
#ifdef CONFIG_OF
static const struct of_device_id gt9xx_dt_match[] = { {.compatible = "mediatek,cap_touch"}, {},};//mediatek,cap_touch_gt by yifaguo
#endif
...
s32 i2c_read_bytes_dma(struct i2c_client *client, u16 addr, u8 *rxbuf, s32 len)
{
		...
		while (left > 0) {
			...
			if (ret < 0) {
				...
				#ifdef CONFIG_HUAWEI_DSM
				tpd_gt9xx_dsm_report_err(DSM_TP_I2C_RW_ERROR_NO, ret);
				#endif
				return -1;
			}
	...
	while (left > 0) {
		...
		while (i2c_transfer(client->adapter, &msg[0], 2) != 2) {
			...
			if (retry == 3) {
				...
				#ifdef CONFIG_HUAWEI_DSM
				tpd_gt9xx_dsm_report_err(DSM_TP_I2C_RW_ERROR_NO, retry);
				#endif
				return -1;
			}
		}
...
static int gtp_send_chr_cfg(struct i2c_client *client)
{
	...
	if (pnl_init_error) {
		GTP_INFO("Error occurred in init_panel, no config sent for charge!");
		return 0;
	}
...
s32 gtp_init_panel(struct i2c_client *client)
{
	...
	static u8 cfg_info_group0[] = CTP_CFG_GROUP0_NORMAL;
	static u8 cfg_info_group0_normal[] = CTP_CFG_GROUP0_NORMAL;
	static u8 cfg_info_group0_high[] = CTP_CFG_GROUP0_HIGH;
	static u8 cfg_info_group1[] = CTP_CFG_GROUP;
	static u8 cfg_info_group2[] = CTP_CFG_GROUP;
	static u8 cfg_info_group3[] = CTP_CFG_GROUP;
	static u8 cfg_info_group4[] = CTP_CFG_GROUP;
	static u8 cfg_info_group5[] = CTP_CFG_GROUP5_NORMAL;
	static u8 cfg_info_group5_normal[] = CTP_CFG_GROUP5_NORMAL;
	static u8 cfg_info_group5_high[] = CTP_CFG_GROUP5_HIGH;

	count = sizeof(cfg_info_group0)/sizeof(cfg_info_group0[0]);

	if(gt615_sensitivity == 0){
		for(j = 0; j < count; j++){
			cfg_info_group0[j] = cfg_info_group0_normal[j];
			cfg_info_group5[j] = cfg_info_group5_normal[j];
		}
	}else if(gt615_sensitivity == 1){
		for(j = 0; j < count; j++){
			cfg_info_group0[j] = cfg_info_group0_high[j];
			cfg_info_group5[j] = cfg_info_group5_high[j];
		}
	}
	...
	const u8 cfg_grp1_charger[] = CTP_CFG_GROUP;
	const u8 cfg_grp2_charger[] = CTP_CFG_GROUP;
	const u8 cfg_grp3_charger[] = CTP_CFG_GROUP;
	const u8 cfg_grp4_charger[] = CTP_CFG_GROUP;
...
#else
	{
		...
		if (ret == SUCCESS) {
			GTP_ERROR("CFG_CONFIG_GROUP%d Config Version: %d, 0x%02X; IC Config Version: %d, 0x%02X",
			sensor_id, send_cfg_buf[sensor_id][0], send_cfg_buf[sensor_id][0], opr_buf[0], opr_buf[0]);
#ifdef GTP_MUTI_FW_COMPATIBLE
			if(gtp_hw_info[global_sensor_id].vendor_name!=NULL)
				tp_devinfo_gt = gtp_hw_info[global_sensor_id].vendor_name;
			else
				tp_devinfo_gt = "GT615,unkown";
#else
			if(global_sensor_id == 0)
				tp_devinfo_gt = "GT615,BOOYI";
			else if(global_sensor_id == 5)
				tp_devinfo_gt = "GT615,DJN";
			else
				tp_devinfo_gt = "GT615,unknow"
#endif
			gt_cfg_version =  send_cfg_buf[sensor_id][0];
			flash_cfg_version = opr_buf[0];
			gt_ic_cfg_version = opr_buf[0];
...
static s8 gtp_i2c_test(struct i2c_client *client)
{
	...
	if ((!ret) && (hw_info == 0x00900600)){
		...
		GTP_ERROR("GTP i2c test failed,GTP_REG_HW_INFO : %08X", hw_info);
		return -1;
	}
}
...
reset_proc:
	...
	/*power isn't use dtsi define "vtouch-supply = <&mt_pmic_vgp1_ldo_reg>"*///verified by yifaguo 2016.06.28
...
#ifdef GTP_MUTI_FW_COMPATIBLE
	if(global_sensor_id <= GTP_SENSOR_ID_MAX)
	{
		if(gtp_hw_info[global_sensor_id].vendor_name!=NULL)
		{
...
{
	//gtp_charger_switch(0);//closed by yifaguo
}

static int gtp_read_info(struct i2c_client *client)
{
	int ret1 = -1, ret2 = -1;
	u8 config = 0;
	u8 buf[8] = {GTP_REG_VERSION >> 8, GTP_REG_VERSION & 0xff};

	ret1 = gtp_i2c_read_dbl_check(client, GTP_REG_CONFIG_DATA, &config, 1);
	if (ret1 == SUCCESS) {
		gt_ic_cfg_version = config;
	} else {
		GTP_ERROR("GTP read ic config version in handler fail!\n");
	}

	ret2 = gtp_i2c_read(client, buf, sizeof(buf));
	if (ret2 >= 0) {
		if (tp_firmware_kbuf) {
		...
		if (!tp_firmware_kbuf) {
			GTP_ERROR("gtxx kmalloc buffer error");
			ret2 = -1;
		}

		tp_firmware_gt = tp_firmware_kbuf;
		sprintf(tp_firmware_gt,"version:%02x%02x, cfg:%x",buf[7], buf[6], gt_ic_cfg_version);
		gt_firmware_ver_low = buf[6];
		gt_firmware_ver_high = buf[7];

	}else{
		GTP_ERROR("GTP read firmware version in handler fail\n");
	}

	if(ret1 == SUCCESS && ret2 >= 0)
		return 0;
	else
		return -1;
}
...
#ifdef CONFIG_GTP_CHARGER_DETECT
	gtp_charger_switch(0);	/* force update */
#endif
		mutex_lock(&i2c_access);
		gtp_irq_disable();
		...
		if (key_value || pre_key) {
			//for (i = 0; i < TPD_KEY_COUNT; i++) {//closed by yifaguo 2016.06.28
...
#endif
			GTP_DEBUG("Touch Release! cfg version:%x, ic cfg version: %x, firmware version:%x%x\n",
				gt_cfg_version,gt_ic_cfg_version,gt_firmware_ver_high,gt_firmware_ver_low);
			tpd_up(0, 0, 0);
		} else {
...
exit_unlock:
	if(tpd_info_flag == -1){
		ret = gtp_read_info(i2c_client_point);
		if(ret == 0){
			tpd_info_flag = 0;
			ctp_hwinfo_flag = 1;
		}
...
/*power isn't use dtsi define "vtouch-supply = <&mt_pmic_vgp1_ldo_reg>"*///verified by yifaguo 2016.06.28
	...
	retval = i2c_add_driver(&tpd_i2c_driver);//modify when 2016.06.29
	GTP_INFO("add i2c driver retval = %d",retval);
	if (retval != 0) {
	...
	if (tpd_load_status == 0)	{
		printk("|%s|%d|tpd_load_status == 0|yifaguo",__func__,__LINE__);//add by yifaguo
...
static s8 gtp_enter_sleep(struct i2c_client *client)
{
	...
	s32 ret = 0;//add by yifaguo 2016.06.28
	gtp_gpio_output(GTP_RST_GPIO, 0);
	msleep(20);
...
/* Skip this code block if chip is not flashless ic */
#ifdef CONFIG_GTP_COMPATIBLE_MODE
	if (CHIP_TYPE_GT9F == gtp_chip_type) {
		u8 opr_buf[2] = {0};
...
/* wakeup touch ic by int pin or hw reset */
	while (retry++ < 3) {
#ifdef CONFIG_GTP_GESTURE_WAKEUP
		if (1) {
...
#else
		{
			...
			/* failed to wakeup by int pin, perform hw reset */
			need_reset = true;
		}
#endif
	} /* end while */
...
static ssize_t gt615_sensitivity_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", gt615_sensitivity_flag);
}

static ssize_t gt615_sensitivity_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	int ret = 0;
	if(!strcmp(buf,"0\n")){
		gt615_sensitivity = 0;
		GTP_ERROR("mtk-tpd:gt615_sensitivity is %d\n",gt615_sensitivity);
	}
	else if(!strcmp(buf,"1\n")){
		gt615_sensitivity = 1;
		GTP_ERROR("mtk-tpd:gt615_sensitivity is %d\n",gt615_sensitivity);
	}else{
		GTP_ERROR("mtk-tpd:gt615_sensitivity is invalid %s\n",buf);
		gt615_sensitivity_flag = -1;
		return size;
	}
	...
	if (ret < 0) {
		gt615_sensitivity_flag = -1;
		GTP_ERROR("GTP send config failed.\n");
	}else{
		gt615_sensitivity_flag = gt615_sensitivity;
		GTP_ERROR("GTP send config success.\n");
	}

	return size;
}

static DEVICE_ATTR(ctp_sensitivity, 0664, gt615_sensitivity_show, gt615_sensitivity_store);

static struct device_attribute *gt615_attrs[] = {
	&dev_attr_ctp_sensitivity,
};

static struct tpd_driver_t tpd_device_driver = {
		.tpd_device_name = "gt615",
		.tpd_local_init = tpd_local_init,
		.suspend = tpd_suspend,
		.resume = tpd_resume,
		.attrs = {
			.attr = gt615_attrs,
			.num  = ARRAY_SIZE(gt615_attrs),
	},
};
