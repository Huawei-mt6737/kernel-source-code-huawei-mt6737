...
unsigned char GTP_VENDOR0_FW[] = {
	#include "HQ_AL828_BOOYI_V2056_GTP.i"
};
unsigned char GTP_VENDOR1_FW[] = {
	//#include "HQ_AL828_DJN_V2054_GTP.i"
};
unsigned char GTP_VENDOR2_FW[] = {
	//#include "HQ_AL828_DJN_V2054_GTP.i"
};
unsigned char GTP_VENDOR3_FW[] = {
	//#include "HQ_AL828_DJN_V2054_GTP.i"
};
unsigned char GTP_VENDOR4_FW[] = {
	//#include "HQ_AL828_DJN_V2054_GTP.i"
};
unsigned char GTP_VENDOR5_FW[] = {
	#include "HQ_AL828_DJN_V2055_GTP.i"
};

struct gtp_hw_info_type gtp_hw_info[GTP_SENSOR_ID_MAX+1] = {
	{
		.vendor_name = "GT615,BOOYI",
		.fw_length = sizeof(GTP_VENDOR0_FW)/sizeof(GTP_VENDOR0_FW[0]),
		.gtp_firmware = GTP_VENDOR0_FW,
	},
	{
		.vendor_name = NULL,
		.fw_length = sizeof(GTP_VENDOR1_FW)/sizeof(GTP_VENDOR1_FW[0]),
		.gtp_firmware = GTP_VENDOR1_FW,
	},
	{
		.vendor_name = NULL,
		.fw_length = sizeof(GTP_VENDOR2_FW)/sizeof(GTP_VENDOR2_FW[0]),
		.gtp_firmware = GTP_VENDOR2_FW,
	},
	{
		.vendor_name = NULL,
		.fw_length = sizeof(GTP_VENDOR3_FW)/sizeof(GTP_VENDOR3_FW[0]),
		.gtp_firmware = GTP_VENDOR3_FW,
	},
	{
		.vendor_name = NULL,
		.fw_length = sizeof(GTP_VENDOR4_FW)/sizeof(GTP_VENDOR4_FW[0]),
		.gtp_firmware = GTP_VENDOR4_FW,
	},
	{
		.vendor_name = "GT615,DJN",
		.fw_length = sizeof(GTP_VENDOR5_FW)/sizeof(GTP_VENDOR5_FW[0]),
		.gtp_firmware = GTP_VENDOR5_FW,
	}
};
unsigned char *gtp_default_FW = NULL;
#else
...
static s8 gup_update_config(struct i2c_client *client)
{
		...
		while (i++ < 5) {
			...
			tpd_gt9xx_dsm_report_err(DSM_TP_FWUPDATE_ERROR_NO, ret);
		#endif
		}
...
#ifdef GTP_MUTI_FW_COMPATIBLE
			update_msg.fw_total_len = gtp_hw_info[global_sensor_id].fw_length - FW_HEAD_LENGTH;
			if (update_msg.fw_total_len <
			( FW_SECTION_LENGTH*4+FW_DSP_ISP_LENGTH+FW_DSP_LENGTH+FW_BOOT_LENGTH)) {
...
s32 gup_update_proc(void *dir)
{
...
#ifdef GTP_MUTI_FW_COMPATIBLE
			gtp_default_FW= gtp_hw_info[global_sensor_id].gtp_firmware;
#endif
#ifdef CONFIG_GTP_AUTO_UPDATE
		if (1 == searching_file) {
			u8 timeout = 0;
		...
		#ifdef CONFIG_HUAWEI_DSM
			tpd_gt9xx_dsm_report_err(DSM_TP_FWUPDATE_ERROR_NO, retry);
		#endif
			update_ret = FAIL;
		} else {
			update_ret = SUCCESS;
