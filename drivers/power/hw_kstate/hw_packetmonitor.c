...
static kuid_t errorUid = {ERROR_UID};
...
static UID_TREE_NODE *search_uid(kuid_t uid)
{
	...
	while (!IS_ERR_OR_NULL(node)) {
		...
		if (uid.val < ut_node->uid_info.uid.val) {
			node = node->rb_left;
		} else if (uid.val > ut_node->uid_info.uid.val) {
			node = node->rb_right;
		} else {
			return ut_node;
		}
...
static UID_TREE_NODE *add_uid_tree_node(UID_MATCH_INFO *uid_info)
{
	...
	while (!IS_ERR_OR_NULL(*tmp)) {
		...
		if (ut_node->uid_info.uid.val < tmp_node->uid_info.uid.val) {
			tmp = &((*tmp)->rb_left);
 		} else if (ut_node->uid_info.uid.val > tmp_node->uid_info.uid.val) {
			tmp = &((*tmp)->rb_right);
		} else {
...
static inline void netfilter_monitor_func(struct sk_buff *skb, u8 ip_version, int dir)
{
	...
	if (mnt_info.skb_info.uid_info.uid.val >= UID_MIN_BOUNDARY) {
		/*uid is valid*/
...
static unsigned int hook_ipv4_local_in_cb(const struct nf_hook_ops *ops,
												struct sk_buff *skb,
...
//	printk("jiangtao, hook_ipv4_local_in_cb");
	return NF_ACCEPT;
}
...
static unsigned int hook_ipv4_local_out_cb(const struct nf_hook_ops *ops,
												struct sk_buff *skb,
												const struct net_device *in,
												const struct net_device *out,
												int (*okfn)(struct sk_buff*))
...
//		printk("jiangtao, hook_ipv4_local_out_cb");
	return NF_ACCEPT;
}
 ...
 *						 sk_buff continue to be transfered
*/
//jiangtao static unsigned int hook_ipv6_local_in_cb(unsigned int hooknum,
static unsigned int hook_ipv6_local_in_cb(const struct nf_hook_ops *ops,
												struct sk_buff *skb,
												const struct net_device *in,
												const struct net_device *out,
												int (*okfn)(struct sk_buff*))
...
//			printk("jiangtao, hook_ipv6_local_in_cb");
	return NF_ACCEPT;
}
...
static unsigned int hook_ipv6_local_out_cb(const struct nf_hook_ops *ops,
												struct sk_buff *skb,
												const struct net_device *in,
												const struct net_device *out,
												int (*okfn)(struct sk_buff*))
...
//				printk("jiangtao, hook_ipv6_local_out_cb");
	return NF_ACCEPT;
}
