...
#include "hwinfo.h"
extern struct hwinfo *battery_hwinfo;  // modify by zjy
#endif
...
#define BATTERY_SUNWODA 0
#define BATTERY_SCUDB_LG  1
#define BATTERY_DESAY  2
#define BATTERY_COSLIGHT  3
#define BATTERY_SCUDB_GY  4
static char *bat_name[]= {"SUNWODA","SCUDB_LG","DESAY","COSLIGHT","SCUDB_GY","UNKNOWN"};

unsigned int g_battery_id_voltage[4] = {50000,600000,900000,1100000};
extern int IMM_GetOneChannelValue_Cali(int Channel, int *voltage);
unsigned int g_fg_battery_id = 0;
unsigned int g_Q_MAX_POS_50[] = {2945, 2948,2924, 2945,2948};
unsigned int g_Q_MAX_POS_25[] = {2935, 2920,2930, 2935,2930};
unsigned int g_Q_MAX_POS_0[] = {2633, 2752,2614, 2633,2930};
unsigned int g_Q_MAX_NEG_10[] = {2100, 2225,2150, 2120,2225};
unsigned int g_Q_MAX_POS_50_H_CURRENT[] = {2945, 2929,2912, 2945,2929};
unsigned int g_Q_MAX_POS_25_H_CURRENT[] = {2980, 2923,2930, 2954,2930};
unsigned int g_Q_MAX_POS_0_H_CURRENT[] = {2633, 2752,2614, 2633,2752};
unsigned int g_Q_MAX_NEG_10_H_CURRENT[] = {1590, 2225,1574, 1487,2225};
int battery_not_match = 0;
char* fgauge_get_battery_manufacturer(void)
{
	return g_battery_manufacturer[g_fg_battery_id];
}
...
void fgauge_get_profile_id(void)
{
	...
	printk("battery id id_volt is %d \n",id_volt);
	if (ret != 0)
...
#ifdef CONFIG_HUAWEI_DSM
     dsm_batt_mt_id_error=0;
#endif
	if ((340000 > id_volt)&&(id_volt > 50000)){
		g_fg_battery_id = BATTERY_COSLIGHT;
		printk("g_fg_battery_id is (%d)\n", BATTERY_COSLIGHT);
	}
	else if ((500000 > id_volt)&&(id_volt > 340000)){
		g_fg_battery_id = BATTERY_DESAY;
		printk("g_fg_battery_id is (%d)\n", BATTERY_DESAY);
	}
	else if ((700000 > id_volt)&&(id_volt > 500000)){
		g_fg_battery_id = BATTERY_SCUDB_GY;
		printk("g_fg_battery_id is (%d)\n", BATTERY_DESAY);
	}
	else if ((900000 > id_volt)&&(id_volt > 700000)){
		g_fg_battery_id = BATTERY_SUNWODA;
		printk("g_fg_battery_id is (%d)\n", BATTERY_SUNWODA);
	}
	else if ((1100000 > id_volt)&&(id_volt > 900000)){
		g_fg_battery_id = BATTERY_SCUDB_LG;
		printk("g_fg_battery_id is (%d)\n", BATTERY_SCUDB_LG);
	}
	else 
		{
			battery_not_match = 1;
			#ifdef CONFIG_HUAWEI_DSM
            dsm_batt_mt_id_error=1;
            #endif
			printk("[fgauge_get_profile_id]Battery is not match!, id_volt (%d)", id_volt);
	}
...
int __batt_meter_init_cust_data_from_cust_header(void)
{
	...
	printk("batt_meter_cust_data.cust_r_sense=%d",batt_meter_cust_data.cust_r_sense);
#endif
...
{
	return sizeof(battery_profile_temperature[g_fg_battery_id]) / sizeof(BATTERY_PROFILE_STRUCT);
}

int fgauge_get_saddles_r_table(void)
{
	return sizeof(r_profile_temperature[g_fg_battery_id]) / sizeof(R_PROFILE_STRUCT);
}
...
BATTERY_PROFILE_STRUCT_P fgauge_get_profile(unsigned int temperature)
{
	...
	case TEMPERATURE_T3:
		...
		/*break;*/
	case TEMPERATURE_T:
		return &battery_profile_temperature[g_fg_battery_id][0];
		/*break;*/
	default:
		return NULL;
		/*break;*/
...
R_PROFILE_STRUCT_P fgauge_get_profile_r_table(unsigned int temperature)
{
	...
	case TEMPERATURE_T3:
		...
		/*break;*/
	case TEMPERATURE_T:
		return &r_profile_temperature[g_fg_battery_id][0];
		/*break;*/
	default:
		return NULL;
		/*break;*/
...
void fgauge_construct_battery_profile_init(void)
{
	...
	printk ("wcl entry fgauge_construct_battery_profile_init\r\n");
...
void fgauge_construct_battery_profile(signed int temperature, BATTERY_PROFILE_STRUCT_P temp_profile_p)
{
	...
	printk("wcl already entry construct battery profile <DOD,Voltage> \r\n");
	if (temperature <= batt_meter_cust_data.temperature_t1) {
	...
	saddles = fgauge_get_saddles();
	printk ("wcl <profile> saddles=%d size is 62? \r\n",saddles);
	...
	for (i = 0; i < saddles; i++) {
		printk( "<DOD,Voltage> at %d = <%d,%d>(%d)\r\n",
			 temperature, (temp_profile_p + i)->percentage,
			 (temp_profile_p + i)->voltage,i);
	}
...
void fgauge_construct_r_table_profile(signed int temperature, R_PROFILE_STRUCT_P temp_profile_p)
{
	...
	saddles = fgauge_get_saddles_r_table();
	printk ("wcl <r_table_profile> saddles=%d size is 62? \r\n",saddles);
...
//#if defined(BATTERY_DEBUG)
	for (i = 0; i < saddles; i++) {
		printk("<Rbat,VBAT> at %d = <%d,%d>(%d)\r\n",
			 temperature, (temp_profile_p + i)->resistance,
			 (temp_profile_p + i)->voltage,i);
	}
//#endif
}
int is_reconstruct_zcv = 0;
void fgauge_construct_table_by_temp(void)
{
	...
	if (avg_temp != last_temp) {
		bm_print(BM_LOG_FULL,
			 ...
			 last_temp, avg_temp);
		bm_print(BM_LOG_CRTI, "[%s] bat_id:%d, reconstruct zcv from %d to %d\n", __func__, g_fg_battery_id, last_temp, avg_temp);
		is_reconstruct_zcv = 1;
		fgauge_construct_r_table_profile(curr_temp,
						 fgauge_get_profile_r_table
						 (batt_meter_cust_data.temperature_t));
		printk("wcl ready construct battery profile <DOD,Voltage> in fgauge_construct_table_by_temp \r\n");
...
#if 1 //defined(EXTERNAL_SWCHR_SUPPORT)
...
/*
	...
	{
*/
sprintf(doddbg,"rtc:%d hwocv:(%d:%d) swocv:(%d,%d) battery_remove:%d %d %d %d,reason:%d,boot:%d",g_rtc_fg_soc,gFG_voltage,gFG_capacity_by_v,gFG_voltage_init,gFG_capacity_by_v_init,is_battery_remove_pmic(),
		CUST_POWERON_DELTA_CAPACITY_TOLRANCE,CUST_POWERON_LOW_CAPACITY_TOLRANCE,CUST_POWERON_DELTA_HW_SW_OCV_CAPACITY_TOLRANCE,get_boot_reason(),get_boot_mode());
...
if (((g_rtc_fg_soc != 0)
		...
		{
			gFG_capacity_by_v = g_rtc_fg_soc;
			printk("use rtc data boot_reason:%d, boot_mode:%d\n", get_boot_reason(), get_boot_mode());
		} else {
...
/* R-Table (First Time) */
	...
	printk("wcl ready construct battery profile <DOD,Voltage> in fgauge_get_dod0 \r\n");
	fgauge_construct_battery_profile(temperature, profile_p);
...
signed int battery_meter_get_charging_current_imm(void)
{
	...
	printk("g_I_SENSE_offset = %d ,batt_meter_cust_data.cust_r_sense =%d\n",g_I_SENSE_offset,CUST_R_SENSE);
	return ICharging;
#endif
}
...
signed int battery_meter_get_charging_current(void)
{
	...
	battery_log(BAT_LOG_CRTI, "[g_Get_I_Charging:BAT_SENSE]\r\n");
	for (i = 0; i < repeat; i++)
	...
	battery_log(BAT_LOG_CRTI, "[g_Get_I_Charging:I_SENSE]\r\n");
	for (i = 0; i < repeat; i++)
	...
	battery_log(BAT_LOG_CRTI, "[g_Get_I_Charging] ADC_BAT_SENSE=%d\r\n", ADC_BAT_SENSE);
	...
	battery_log(BAT_LOG_CRTI, "[g_Get_I_Charging] ADC_I_SENSE(Before)=%d\r\n", ADC_I_SENSE);
	...
	printk("g_I_SENSE_offset = %d,ADC_I_SENSE = %d,ADC_BAT_SENSE = %d,batt_meter_cust_data.cust_r_sense =%d,ICharging is %d\n",g_I_SENSE_offset,ADC_I_SENSE,ADC_BAT_SENSE,batt_meter_cust_data.cust_r_sense,ICharging);
	return ICharging;
#else
	return 0;
#endif
...
signed int battery_meter_initial(void)
{
	...
	printk("zcy battery_meter_initial!!!\n");
	mutex_lock(&FGADC_mutex);
	if (meter_initilized == KAL_FALSE) {
#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
		fgauge_get_profile_id();
		printk("battery id is %d \n",g_fg_battery_id);
#endif

#ifdef CONFIG_HQ_HWINFO
	if(g_fg_battery_id <= 5)
		hwinfo_set_devinfo(battery_hwinfo ,"%s", bat_name[g_fg_battery_id]);
	printk("battery hwinfo g_fg_battery_id=%d,battery_hwinfo=%s \n\r",g_fg_battery_id,battery_hwinfo->devinfo);
	if(!battery_hwinfo->devinfo){	
		battery_hwinfo->devinfo=bat_name[g_fg_battery_id];
		printk("battery hwinfo battery_hwinfo->devinfo=%s \n\r",battery_hwinfo->devinfo);
		}
	printk("battery hwinfo g_fg_battery_id=%d,battery_hwinfo=%s \n\r",g_fg_battery_id,battery_hwinfo->devinfo);	
#endif
/*
#ifdef CONFIG_HQ_HWINFO
	if(g_fg_battery_id == BATTERY_COSLIGHT)
		hwinfo_set_devinfo(battery_hwinfo ,"%s", bat_name[3]);
	else if(g_fg_battery_id == BATTERY_DESAY)
		hwinfo_set_devinfo(battery_hwinfo ,"%s", bat_name[2]);
	else if (g_fg_battery_id == BATTERY_SUNWODA)
		hwinfo_set_devinfo(battery_hwinfo ,"%s", bat_name[0]);
	else if (g_fg_battery_id == BATTERY_SCUDB_LG)
		hwinfo_set_devinfo(battery_hwinfo ,"%s", bat_name[1]);
	else if (g_fg_battery_id == BATTERY_SCUDB_GY)
		hwinfo_set_devinfo(battery_hwinfo ,"%s", bat_name[4]);
	else
		hwinfo_set_devinfo(battery_hwinfo ,"%s", bat_name[5]);
	printk("battery hwinfo g_fg_battery_id=%d \n\r",g_fg_battery_id);
#endif
*/
#if defined(SOC_BY_AUXADC)
		g_auxadc_solution = 1;
