...
#include <linux/wakelock.h>
 ...
 /* cut off to full */
int dm_charge_vol=0;
int dm_charge_avgv=0;
int get_charge_vol=0;
Charge_volsize_enum g_charge_vol_status;
unsigned int max_topoff_charging_time;
...
static int g_jeita_first_set_cv_cnt=0;
...
extern kal_bool dpm_first_select_cc;
extern kal_bool find_dpm_cc;
extern kal_bool up_status;
extern kal_bool down_status;
extern unsigned int cc_rank_value;
#endif
extern int g_lcm_status;
//extern g_call_state;
...
static void __init_charging_varaibles(void)
{
	...
	if (init_flag == 0) {
		...
		v_cc2topoff_threshold = V_CC2TOPOFF_THRES;
		ulc_cv_charging_current = AC_CHARGER_CURRENT;
...
static BATTERY_VOLTAGE_ENUM select_jeita_cv(void)
{
	...
	} else if ((g_temp_status == TEMP_POS_0_TO_POS_5)||(g_temp_status == TEMP_POS_5_TO_POS_10)){
...
PMU_STATUS do_jeita_state_machine(void)
{
	...
	if (BMT_status.temperature >= TEMP_POS_60_THRESHOLD) {
		...
		if ((g_temp_status == TEMP_ABOVE_POS_60)
			...
			if (BMT_status.bat_vol >= V_CHARGER_45TO60_WARNING) {
				is_recharge_4_1v = 1;
			battery_log(BAT_LOG_CRTI,
					    "[BATTERY] zjy-Battery Temperature between %d and %d BMT_status.bat_vol:%d no charge!!\n",
					    TEMP_POS_45_THRESHOLD, TEMP_POS_60_THRESHOLD, BMT_status.bat_vol);
				return PMU_STATUS_FAIL;
			} else if ((BMT_status.bat_vol >= V_CHARGER_45TO60_RECHAR) && (1 == is_recharge_4_1v))  {
				battery_log(BAT_LOG_CRTI,
					    "[BATTERY] zjy-Battery Temperature between %d and %d BMT_status.bat_vol:%d not charge!!\n",
					    TEMP_POS_45_THRESHOLD, TEMP_POS_60_THRESHOLD, BMT_status.bat_vol);
				return PMU_STATUS_FAIL;
			} else {	
				is_recharge_4_1v = 0;
				battery_log(BAT_LOG_CRTI,
						    "[BATTERY] Battery Temperature between %d and %d, BMT_status.bat_vol:%d, %d!!\n\r",
						    TEMP_POS_45_THRESHOLD, TEMP_POS_60_THRESHOLD, BMT_status.bat_vol, BATTERY_VOLT_04_100000_V);
			...
			v_cc2topoff_threshold = JEITA_TEMP_POS_45_TO_POS_60_CC2TOPOFF_THRESHOLD;
				charging_full_current = JEITA_TEMP_POS_45_TO_POS_60_FULL_CURRENT;
		}
		}
		...
		if (((g_temp_status == TEMP_POS_45_TO_POS_60)
			...
			charging_full_current = JEITA_TEMP_POS_10_TO_POS_45_FULL_CURRENT;
		}
		...
		if ((g_temp_status == TEMP_NEG_10_TO_POS_0 || g_temp_status == TEMP_BELOW_NEG_10)
			...
			g_temp_status = TEMP_POS_0_TO_POS_5;
			g_jeita_recharging_voltage = JEITA_TEMP_POS_0_TO_POS_10_RECHARGE_VOLTAGE;
			v_cc2topoff_threshold = JEITA_TEMP_POS_0_TO_POS_10_CC2TOPOFF_THRESHOLD;
			charging_full_current = JEITA_TEMP_POS_10_TO_POS_45_FULL_CURRENT;
		}
	...
	g_jeita_first_set_cv_cnt++;
		battery_xlog_printk(BAT_LOG_CRTI,
				    "[BATTERY] wcl g_temp_status:%d, g_jeita_first_set_cv:%d!!\n\r",
				    g_temp_status,g_jeita_first_set_cv_cnt);
	if ((g_jeita_first_set_cv_cnt % 3) == 0) {
		is_cv_init = KAL_FALSE;
		g_jeita_first_set_cv_cnt = 0;
	}
	if (g_temp_status != previous_g_temp_status || is_cv_init == KAL_FALSE) {
	cv_voltage = select_jeita_cv();
	printk("[BATTERY] setcv cv_voltage=%d, g_temp_status:%d, g_jeita_first_set_cv:%d,charging_full_current:%d,g_jeita_recharging_voltage:%d !!\n\r",
				    cv_voltage,g_temp_status, cv_voltage, charging_full_current, g_jeita_recharging_voltage);
...
static void set_jeita_charging_current(void)
{
	...
	if (g_temp_status == TEMP_BELOW_NEG_10) {
		g_temp_CC_value = CHARGE_CURRENT_0_00_MA;	/* for low temp */
		...
		battery_log(BAT_LOG_CRTI, "[BATTERY] JEITA set charging current : %d\r\n",
			    g_temp_CC_value);
	}else if(g_temp_status == TEMP_POS_0_TO_POS_5) {
		g_temp_CC_value = CHARGE_CURRENT_300_00_MA;	/* 0C ~ 5C */
		battery_log(BAT_LOG_CRTI, "[BATTERY] JEITA set charging current0-5C: %d\r\n",
				    g_temp_CC_value);
	}else if((g_temp_status == TEMP_POS_5_TO_POS_10)||(g_temp_status == TEMP_POS_45_TO_POS_60)) {//5~10 and 45~60
#ifdef HUAWEI_DYNAMIC_CHARGER
		printk("entery AC 5~10||45~60C g_temp_status=%d,dpm_first_select_cc=%d,find_dpm_cc=%d,cc_rank_value=%d,g_charge_vol_status=%d,\n",g_temp_status,dpm_first_select_cc,find_dpm_cc,cc_rank_value,g_charge_vol_status);
		if(dpm_first_select_cc==KAL_TRUE){
				g_temp_CC_value= STANDARD_DM_CHARGER2;
				dpm_first_select_cc= KAL_FALSE;
				cc_rank_value=2;
		}else if(find_dpm_cc==KAL_FALSE){
		printk("entery dpm select cc,find_dpm_cc1=%d,dpm_first_select_cc=%d,cc_rank_value=%d,g_charge_vol_status=%d,\n",find_dpm_cc,dpm_first_select_cc,cc_rank_value,g_charge_vol_status);
			 if(g_charge_vol_status == g_charge_vol_3){
					if(cc_rank_value >= 3){
						cc_rank_value= 3;
						find_dpm_cc=KAL_TRUE;
					}else{
						up_status=KAL_TRUE;
							cc_rank_value++;
						if(down_status==KAL_TRUE){
							find_dpm_cc=KAL_TRUE;
							cc_rank_value--;
							printk("down->up find_dpm_cc=%d,cc_rank_value=%d \n",find_dpm_cc,cc_rank_value);
							}
					}
					if(cc_rank_value==1){
						g_temp_CC_value = STANDARD_DM_CHARGER1;
					}else if(cc_rank_value==2){
						g_temp_CC_value = STANDARD_DM_CHARGER2;
					}else if(cc_rank_value==3){
						g_temp_CC_value = STANDARD_DM_CHARGER3;
					}
				printk("3 find_dpm_cc=%d,cc_rank_value=%d,g_temp_CC_value=%d\n",find_dpm_cc,cc_rank_value,g_temp_CC_value);
			}else if(g_charge_vol_status == g_charge_vol_2){
					if(cc_rank_value == 2)
						{
						up_status=KAL_TRUE;
						cc_rank_value++;
			...
			g_temp_CC_value = STANDARD_DM_CHARGER2;
				...
				printk("2 find_dpm_cc=%d,cc_rank_value=%d,g_temp_CC_value=%d\n",find_dpm_cc,cc_rank_value,g_temp_CC_value);
		}else if(g_charge_vol_status == g_charge_vol_1){
					if(cc_rank_value <= 1){
						cc_rank_value= 1;
						find_dpm_cc=KAL_TRUE;
					}else{
						down_status=KAL_TRUE;
							cc_rank_value--;
						if(up_status==KAL_TRUE)
							find_dpm_cc=KAL_TRUE;
						printk("up->down find_dpm_cc=%d,cc_rank_value=%d \n",find_dpm_cc,cc_rank_value);
						}
					if(cc_rank_value==1){
			g_temp_CC_value = STANDARD_DM_CHARGER1;
					}else if(cc_rank_value==2){
					...
					printk("1 find_dpm_cc=%d,cc_rank_value=%d,g_temp_CC_value=%d\n",find_dpm_cc,cc_rank_value,g_temp_CC_value);
					}
			 }else if (find_dpm_cc==KAL_TRUE){
				if(cc_rank_value==1){
						g_temp_CC_value = STANDARD_DM_CHARGER1;
					}else if(cc_rank_value==2){
						g_temp_CC_value = STANDARD_DM_CHARGER2;
					...
					printk("dpm 45-60||5-10C cc_rank_value==4(%d),select==3(800ma)",cc_rank_value);
						g_temp_CC_value = STANDARD_DM_CHARGER3;//800ma
					}
			printk("end select 45-60||5-10C dpm cc,find_dpm_cc1=%d,cc_rank_value=%d,g_temp_CC_value=%d,\n",find_dpm_cc,cc_rank_value,g_temp_CC_value);
			 }
	printk("[BATTERY] set 5~10||45~60C dynamic char cc(300,550,800)is:%d,cc_rank_value=%d,find_dpm_cc=%d,g_charge_vol_status:%d \r\n",
				    g_temp_CC_value,cc_rank_value,find_dpm_cc,g_charge_vol_status);
//huawei dynamic charging end
#else
		g_temp_CC_value = CHARGE_CURRENT_800_00_MA;	/* 5C ~10C */
		battery_log(BAT_LOG_CRTI, "[BATTERY] JEITA set 5~10C charging current : %d\r\n",
				    g_temp_CC_value);
#endif		
	}else if(g_temp_status == TEMP_POS_10_TO_POS_45) {
	printk("entery AC 10~45C g_temp_status=%d,dpm_first_select_cc=%d,find_dpm_cc=%d,g_charge_vol_status=%d,cc_rank_value=%d,g_charge_vol_status=%d,\n",g_temp_status,dpm_first_select_cc,find_dpm_cc,g_charge_vol_status,cc_rank_value,g_charge_vol_status);
//huawei dynamic charging begin
#ifdef HUAWEI_DYNAMIC_CHARGER 
		if(dpm_first_select_cc==KAL_TRUE){
			printk("first select cc cc_rank_value=%d\n",cc_rank_value);
			g_temp_CC_value= STANDARD_DM_CHARGER2;
			dpm_first_select_cc= KAL_FALSE;
			cc_rank_value=2;
		}else if(find_dpm_cc==KAL_FALSE){
			...
			g_temp_CC_value = STANDARD_DM_CHARGER3;
				}else if(cc_rank_value==4){
					g_temp_CC_value = STANDARD_DM_CHARGER4;
					}
				printk("3 find_dpm_cc=%d,cc_rank_value=%d,g_temp_CC_value=%d\n",find_dpm_cc,cc_rank_value,g_temp_CC_value);
			}else if(g_charge_vol_status == g_charge_vol_2){
				if((cc_rank_value == 2)||(cc_rank_value == 3)){
					up_status=KAL_TRUE;
					cc_rank_value++;
				}else
					find_dpm_cc = KAL_TRUE;
			...
			printk("end dpm select cc,find_dpm_cc1=%d,cc_rank_value=%d,g_temp_CC_value=%d,\n",find_dpm_cc,cc_rank_value,g_temp_CC_value);
		}
		printk("[BATTERY]STANDARD_CHARGER set 10~45C cc(300,550,800,1000)is:%d,cc_rank_value=%d,find_dpm_cc=%d,g_charge_vol_status:%d \r\n",
				    g_temp_CC_value,cc_rank_value,find_dpm_cc,g_charge_vol_status);
//huawei dynamic charging end
#else
		g_temp_CC_value = CHARGE_CURRENT_1000_00_MA;	/* 10C ~ 45C */
		battery_log(BAT_LOG_CRTI, "[BATTERY] JEITA set charging current : %d\r\n", g_temp_CC_value);
#endif		
	}
	//else if(g_temp_status == TEMP_POS_45_TO_POS_60) {
	//	g_temp_CC_value = CHARGE_CURRENT_800_00_MA;	/* 45C ~ 60C */
	//	battery_log(BAT_LOG_CRTI, "[BATTERY] JEITA set charging current : %d\r\n",
	//			    g_temp_CC_value);
	else{
		g_temp_CC_value = CHARGE_CURRENT_0_00_MA;	/* for high temp */
		battery_log(BAT_LOG_CRTI, "[BATTERY] JEITA set charging current : %d\r\n",
			    g_temp_CC_value);
	}
}
...
unsigned int set_bat_charging_current_limit(int current_limit)
{
	battery_log(BAT_LOG_CRTI, "[BATTERY] set_bat_charging_current_limit (%d)\r\n",
	 ...
	 if (current_limit != -1) {
		...
		printk("bcct set current limit=%d \n",g_temp_CC_value);
		save_cc_calue=g_temp_CC_value;
	} else {
...
if (g_ftm_battery_flag) {
		...
		if (BMT_status.charger_type == STANDARD_HOST) {
			...
			if ((g_temp_status == TEMP_BELOW_NEG_10)||(g_temp_status ==TEMP_ABOVE_POS_60)) {
				g_temp_CC_value = CHARGE_CURRENT_0_00_MA;	/* for low temp */
				battery_log(BAT_LOG_CRTI, "[BATTERY] NONSTANDARD_CHARGER JEITA set charging current 0 : %d\r\n",
				    g_temp_CC_value);
			}else if(g_temp_status == TEMP_NEG_10_TO_POS_0) {
				g_temp_CC_value = CHARGE_CURRENT_0_00_MA;	/* for low temp */
				battery_log(BAT_LOG_CRTI, "[BATTERY] NONSTANDARD_CHARGER JEITA set charging current 00 : %d\r\n",
			    g_temp_CC_value);
			}else if(g_temp_status == TEMP_POS_0_TO_POS_5) {
				g_temp_CC_value = CHARGE_CURRENT_300_00_MA;	/* 0C ~ 5C */
				battery_log(BAT_LOG_CRTI, "[BATTERY] NONSTANDARD_CHARGER JEITA set 0~5C charging current : %d\r\n",
				    g_temp_CC_value);
			}else {
//huawei dynamic charging begin
#ifdef HUAWEI_DYNAMIC_CHARGER
			printk("entery nos 10~45C g_temp_status=%d,dpm_first_select_cc=%d,find_dpm_cc=%d,cc_rank_value=%d,g_charge_vol_status=%d,\n",g_temp_status,dpm_first_select_cc,find_dpm_cc,cc_rank_value,g_charge_vol_status);
			if(dpm_first_select_cc==KAL_TRUE){
				g_temp_CC_value= NO_STANDARD_DM_CHARGER2;
				dpm_first_select_cc= KAL_FALSE;
				cc_rank_value=2;
			 }else if(find_dpm_cc==KAL_FALSE){
			 printk("nos entery dpm select cc,find_dpm_cc1=%d,dpm_first_select_cc=%d,cc_rank_value=%d,g_temp_CC_value=%d,\n",find_dpm_cc,dpm_first_select_cc,cc_rank_value,g_temp_CC_value);
			 		if(g_charge_vol_status == g_charge_vol_3){
						if(cc_rank_value >= 3){
						cc_rank_value= 3;
						find_dpm_cc=KAL_TRUE;
					...
					if(down_status==KAL_TRUE){
						find_dpm_cc=KAL_TRUE;
						cc_rank_value--;
						printk("nos down->up find_dpm_cc=%d,cc_rank_value=%d \n",find_dpm_cc,cc_rank_value);
								}
						}
					if(cc_rank_value==1){
						g_temp_CC_value = NO_STANDARD_DM_CHARGER1;
					}else if(cc_rank_value==2){
						g_temp_CC_value = NO_STANDARD_DM_CHARGER2;
					}else if(cc_rank_value==3){
					g_temp_CC_value = NO_STANDARD_DM_CHARGER3;
						}
					printk("nos 3 find_dpm_cc=%d,cc_rank_value=%d,g_temp_CC_value=%d\n",find_dpm_cc,cc_rank_value,g_temp_CC_value);
				}else if(g_charge_vol_status == g_charge_vol_2) {
					if(cc_rank_value == 2){
						up_status=KAL_TRUE;
						cc_rank_value++;
					...
					printk("nos 2 find_dpm_cc=%d,cc_rank_value=%d,g_temp_CC_value=%d\n",find_dpm_cc,cc_rank_value,g_temp_CC_value);
				}else if(g_charge_vol_status == g_charge_vol_1){
					if(cc_rank_value <= 1){
						cc_rank_value= 1;
						find_dpm_cc=KAL_TRUE;
					...
					if(up_status==KAL_TRUE)
						find_dpm_cc=KAL_TRUE;
					printk("nos up->down find_dpm_cc=%d,cc_rank_value=%d \n",find_dpm_cc,cc_rank_value);
						}
					if(cc_rank_value==1){
					g_temp_CC_value = NO_STANDARD_DM_CHARGER1;
					}else if(cc_rank_value==2){
				...
				if(cc_rank_value==1){
					...
					g_temp_CC_value = NO_STANDARD_DM_CHARGER3;
						...
						g_temp_CC_value = NO_STANDARD_DM_CHARGER3;
				printk("nos end select dpm cc,find_dpm_cc=%d,cc_rank_value=%d,g_temp_CC_value=%d,\n",find_dpm_cc,cc_rank_value,g_temp_CC_value);
			 }	
		printk("nos set 10~45C dynamic char cc(300,550,800)is:%d,cc_rank_value=%d,find_dpm_cc=%d,g_charge_vol_status:%d \r\n",
				    g_temp_CC_value,cc_rank_value,find_dpm_cc,g_charge_vol_status);
//huawei dynamic charging end
#else
			g_temp_CC_value = NON_STD_AC_CHARGER_CURRENT;//default
#endif			
				}
...
static unsigned int charging_full_check(void)
{
	...
	if (((BMT_status.bat_vol >= 4360)&&(BMT_status.ZCV>=4360)&&(BMT_status.ICharging<=220)&&(gFG_current <= 650)&&(gFG_Is_Charging == 1))||(BMT_status.ICharging <= charging_full_current)||((BMT_status.bat_vol >= 4400)&& (BMT_status.ICharging <= 150))) {
		printk("bat_vol=%d,ICharging=%d,gFG_current=%d,gFG_Is_Charging=%d\n ",BMT_status.bat_vol,BMT_status.ICharging,gFG_current,gFG_Is_Charging );
		full_check_count++;
		if (6 == full_check_count) {
			status = KAL_TRUE;
			full_check_count = 0;
			battery_log(BAT_LOG_CRTI,
				    "[BATTERY] Battery full, full_check_count=6,(ICharging:%d ma <= full_current:%d ma) OR (bat_vol:%d >= full_vol:4383mv && BMT_status.ICharging:%d <=150),start disable charging on\n",
				    BMT_status.ICharging,charging_full_current,BMT_status.bat_vol,BMT_status.ICharging);
		}
	} else {
		full_check_count = 0;
		battery_log(BAT_LOG_CRTI,
				    "[BATTERY] Battery not full, full_check_count=%d,(ICharging:%d ma,full_current:%d ma ,bat_vol:%d , full_vol:4383mv  charging on \n",
				    full_check_count,BMT_status.ICharging,charging_full_current,BMT_status.bat_vol);
	}
#endif
...
//zcy_add_20160711 add
extern int g_runin_test_enter;
//zcy for battery runin
extern signed int gFG_current;
extern kal_bool gFG_Is_Charging ;
extern signed int battery_meter_get_charging_current(void);
signed int avgsyscurrent = 0;

void pchr_turn_on_charging(void)
{
	unsigned int charging_enable = KAL_TRUE;
	int i=0;
	static signed int sys_current;
	static kal_bool sysBufferFirst = KAL_TRUE;
	static signed int sysBuffer[5];
	static signed int sys_sum,max,min;
	...
	dm_charge_avgv=0;
	get_charge_vol=0;
	g_charge_vol_status = g_charge_vol_3;

	sys_current=( battery_meter_get_charging_current() -gFG_current/10 );
	battery_log(BAT_LOG_CRTI, "[BATTERY] pchr_turn_on_charging()!\r\n");
	battery_log(BAT_LOG_CRTI,"get sys_current =%d,charging_current=%d,gFG_current=%d \n",sys_current,battery_meter_get_charging_current(),gFG_current);
	if ((sysBufferFirst == KAL_TRUE)&&(BMT_status.charger_exist == 1)) {
		for (i = 0; i < 5; i++)
			sysBuffer[i] = sys_current;
		sys_sum =sys_current * 5;
		sysBufferFirst = KAL_FALSE;
	}else if(BMT_status.charger_exist == 0){
		sysBufferFirst = KAL_TRUE;
		avgsyscurrent=0;
		Index=0;
		sys_sum=0;
	}
	if(BMT_status.charger_exist == 1){
	sysBuffer[Index] = sys_current;
	max=sysBuffer[0];
	min=sysBuffer[0];
	sys_sum=0;
	for(i=0;i<5;i++){
	...
	avgsyscurrent = (sys_sum-max-min)/3;
	battery_log(BAT_LOG_CRTI, "get avgsys =%d,sys_sum= %d,max=%d,min=%d,Index =%d\n",avgsyscurrent,sys_sum, max,min,Index);
	Index++;
	if (Index >= 5){
		Index = 0;
		}
	...
	for(i=0;i<5;i++){
			get_charge_vol=battery_meter_get_charger_voltage();
			dm_charge_vol += get_charge_vol;
			battery_log(BAT_LOG_CRTI,"[BATTERY] get_charge_vol instant: %d,dm_charge_vol:(i %d,sum %d)\r\n",
											get_charge_vol,i,dm_charge_vol);
			msleep(100);
		}
	dm_charge_avgv=dm_charge_vol/5;
	battery_log(BAT_LOG_CRTI,"[BATTERY]dm_charge_avgv : %d \r\n",dm_charge_avgv);
#ifdef HUAWEI_DYNAMIC_CHARGER
			if(dm_charge_avgv > 4750){
				g_charge_vol_status= g_charge_vol_3;
			}else if((dm_charge_avgv >= 4500)&&(dm_charge_avgv <= 4750)) {
				g_charge_vol_status = g_charge_vol_2;
			}else {
				g_charge_vol_status = g_charge_vol_1;
					}
		battery_log(BAT_LOG_CRTI, "[BATTERY] get g_charge_vol_status=:%d,\r\n",g_charge_vol_status);
#endif
	...
	if (BMT_status.bat_charging_state == CHR_ERROR) {
		...
		if (get_usb_current_unlimited()) {
			...
			printk("[BATTERY] select_charging_current! cc=%d\n",g_temp_CC_value);
			if((g_bcct_flag == 1)&&(find_dpm_cc == KAL_TRUE)){
				printk("[BATTERY] select_charging_curret_bcct! g_temp_CC_value=%d,save_cc_calue=%d\n",g_temp_CC_value,save_cc_calue);
				select_charging_curret_bcct();
				if(g_temp_CC_value >=save_cc_calue){
					g_temp_CC_value=save_cc_calue;
					printk("[BATTERY] g_temp_CC_value=%d,save_cc_calue=%d\n",g_temp_CC_value,save_cc_calue);
				}
			}
		}
...
printk("find_dpm_cc= %d,charger_exist=%d,g_lcm_status=%d\n", find_dpm_cc,BMT_status.charger_exist,g_lcm_status);
if((find_dpm_cc == KAL_TRUE)&&(BMT_status.charger_exist == 1)){
	printk("before g_call_state and g_lcm_status CC= %d,g_call_state=%d,g_lcm_status=%d \n", g_temp_CC_value,g_call_state,g_lcm_status);
	if((g_call_state==1)&&(g_lcm_status==0)){
		if(g_temp_CC_value >CHARGE_CURRENT_650_00_MA)
			g_temp_CC_value=CHARGE_CURRENT_650_00_MA;
	}
	printk("after g_call_state and g_lcm_status CC= %d \n", g_temp_CC_value);
	

	if((avgsyscurrent>=400)&&(g_lcm_status==1)){
		if(g_temp_CC_value >CHARGE_CURRENT_800_00_MA)
			g_temp_CC_value=CHARGE_CURRENT_800_00_MA;
	}
	printk("avgsyscurrent=%d,g_temp_CC_value = %d  \n", avgsyscurrent,g_temp_CC_value);
...
if (ta_check_ta_control == KAL_FALSE)
	...
	//zcy_add_20160711 start
     	if(g_runin_test_enter)
     	{
         	charging_enable = KAL_FALSE;
         	battery_xlog_printk(BAT_LOG_CRTI, "[BATTERY] g_runin_test_enter is Active, turn off charging !\r\n");
     	}
     //zcy_add_20160711 end
...
PMU_STATUS BAT_TopOffModeAction(void)
{
	...
	if(BMT_status.charger_type == STANDARD_HOST)
	{	
		printk("entery usb topoff charging mode not max topoff timer");
		//max_topoff_charging_time = MAX_USB_CV_CHARGING_TIME;  /* 5h */
		...
		printk("entery no usb topoff charging mode select max topoff timer 3h");
		max_topoff_charging_time = MAX_CV_CHARGING_TIME;  /* 3h */
		if ((BMT_status.TOPOFF_charging_time >= max_topoff_charging_time)
