...
static BATTERY_VOLTAGE_ENUM select_jeita_cv(void)
{
	...
	if (g_temp_status == TEMP_ABOVE_POS_60) {
		...
		cv_voltage = JEITA_TEMP_POS_0_TO_POS_5_CV_VOLTAGE;
...
PMU_STATUS do_jeita_state_machine(void)
{
	...
	if (BMT_status.temperature >= TEMP_POS_60_THRESHOLD) {
		...
		if ((g_temp_status == TEMP_ABOVE_POS_60)
			...
			jeita_status = PMU_STATUS_FAIL;
		             ...
		             if (BMT_status.bat_vol >= V_CHARGER_45TO60_WARNING) {
				        is_recharge_4_1v = 1;
			                battery_log(BAT_LOG_CRTI,
									"[BATTERY] zjy1-Battery Temperature between %d and %d BMT_status.bat_vol:%d no charge!!\n",
					...
					battery_log(BAT_LOG_CRTI,
									"[BATTERY] zjy2-Battery Temperature between %d and %d BMT_status.bat_vol:%d not charge!!\n",
									TEMP_POS_45_THRESHOLD, TEMP_POS_60_THRESHOLD, BMT_status.bat_vol);
					return PMU_STATUS_FAIL;
			       } else {	
					  is_recharge_4_1v = 0;
					battery_log(BAT_LOG_CRTI,"[BATTERY] faj-Battery Temperature between %d and %d, BMT_status.bat_vol:%d, %d!!\n\r",
			...
			batt_cust_data.recharging_voltage = JEITA_TEMP_POS_45_TO_POS_60_RECHARGE_VOLTAGE;
			}
		}
		...
		if (((g_temp_status == TEMP_POS_45_TO_POS_60)
			...
			batt_cust_data.recharging_voltage = JEITA_TEMP_POS_10_TO_POS_45_RECHARGE_VOLTAGE;
		}
			...
			g_temp_status = TEMP_POS_5_TO_POS_10;
			batt_cust_data.recharging_voltage = JEITA_TEMP_POS_0_TO_POS_10_RECHARGE_VOLTAGE;
		...
		if ((g_temp_status == TEMP_NEG_10_TO_POS_0 || g_temp_status == TEMP_BELOW_NEG_10)
			...
			battery_log(BAT_LOG_CRTI,
				    "[BATTERY] Battery Temperature between %d and %d !!\n\r",
				    TEMP_POS_0_THRESHOLD, TEMP_POS_5_THRESHOLD);
		...
		if ((g_temp_status == TEMP_BELOW_NEG_10)
			...
			batt_cust_data.recharging_voltage = JEITA_TEMP_NEG_10_TO_POS_0_RECHARGE_VOLTAGE;
			return PMU_STATUS_FAIL;
		}
	} else {
		battery_log(BAT_LOG_CRTI,
	...
	battery_log(BAT_LOG_CRTI,"[BATTERY] set cv_voltage=%d !!\n\r",cv_voltage);
...
static void set_jeita_charging_current(void)
{
	...
	if (BMT_status.charger_type == STANDARD_HOST)
		return;
			...
			if(BMT_status.charger_type == STANDARD_HOST)
			{
				    if(g_temp_status == TEMP_POS_0_TO_POS_5) {
				g_temp_CC_value = CHARGE_CURRENT_300_00_MA; /* 0C ~15C 900mA*/
				g_temp_input_CC_value = CHARGE_CURRENT_500_00_MA; 
				battery_log(BAT_LOG_CRTI, "[BATTERY] JEITA usb set charging current : %d input_CC_value:%d\r\n",
	...
	if(BMT_status.charger_type == STANDARD_CHARGER){
			if (g_temp_status == TEMP_BELOW_NEG_10) {
				g_temp_CC_value = CHARGE_CURRENT_0_00_MA;	/* below -10C stop charge */
				g_temp_input_CC_value = CHARGE_CURRENT_0_00_MA; 
				battery_log(BAT_LOG_CRTI, "[BATTERY] JEITA set charging current : %d input_CC_value:%d\r\n",
							g_temp_CC_value,g_temp_input_CC_value);
			}else if(g_temp_status == TEMP_NEG_10_TO_POS_0) {
				g_temp_CC_value = CHARGE_CURRENT_0_00_MA; /* -3c ~ 0c 250mA*/
				g_temp_input_CC_value = CHARGE_CURRENT_0_00_MA; 
				battery_log(BAT_LOG_CRTI, "[BATTERY] JEITA set charging current : %d input_CC_value:%d\r\n",
							g_temp_CC_value,g_temp_input_CC_value);
			}else if(g_temp_status == TEMP_POS_0_TO_POS_5) {
				g_temp_CC_value = CHARGE_CURRENT_300_00_MA; /* 0C ~5C 300mA*/
				g_temp_input_CC_value = CHARGE_CURRENT_500_00_MA; 
				battery_log(BAT_LOG_CRTI, "[BATTERY] JEITA set charging current : %d input_CC_value:%d\r\n",
							g_temp_CC_value,g_temp_input_CC_value);
			}else if((g_temp_status == TEMP_POS_5_TO_POS_10) ||(g_temp_status == TEMP_POS_45_TO_POS_60)){
							g_temp_CC_value = CHARGE_CURRENT_800_00_MA;	/* 5C ~10C */
							g_temp_input_CC_value = CHARGE_CURRENT_900_00_MA; 
							battery_log(BAT_LOG_CRTI, "[BATTERY] JEITA set charging current : %d input_CC_value:%d\r\n",
							g_temp_CC_value,g_temp_input_CC_value);
			}else if(g_temp_status == TEMP_POS_10_TO_POS_45) {
					g_temp_CC_value = CHARGE_CURRENT_1800_00_MA;	/* 10C ~ 45C */
...
unsigned int save_CC_value =0;
unsigned int save_input_CC_value=0;
...
*/
	...
	if (g_bcct_flag == 1)
	{  
		if(g_bcct_value == 1024){
		 	battery_log(BAT_LOG_CRTI, "[BATTERY] set_charging_current_bcct g_bcct_flag =%d,g_bcct_value=%d\r\n",g_bcct_flag,g_bcct_value);
	                g_temp_CC_value = g_bcct_value * 100;
			g_temp_input_CC_value = CHARGE_CURRENT_1200_00_MA;
	    
	        }else if(g_bcct_value == 512)
		{
			battery_log(BAT_LOG_CRTI, "[BATTERY] set_charging_current_bcct g_bcct_flag =%d,g_bcct_value=%d\r\n",g_bcct_flag,g_bcct_value);
			g_temp_CC_value = g_bcct_value * 100;
			g_temp_input_CC_value = CHARGE_CURRENT_900_00_MA;
...
void select_charging_current(void)
{
	if (g_ftm_battery_flag) {
		...
		if (BMT_status.charger_type == STANDARD_HOST) {
			...
			//g_temp_input_CC_value = batt_cust_data.non_std_ac_charger_input_current;
...
static unsigned int charging_full_check(void)
{
	...
	if ((BMT_status.bat_vol >= 4395)&&(gFG_current <= 1700)&&(gFG_Is_Charging == 1))
	{	
		full_check_count++;
		battery_log(BAT_LOG_CRTI,
				    "charging_full_check,full_check_count=%d,BMT_status.bat_vol=%d,gFG_current=%d\n",full_check_count,BMT_status.bat_vol,gFG_current);
		if (3 == full_check_count) {
			status = KAL_TRUE;
			full_check_count = 0;
			battery_log(BAT_LOG_CRTI,
				    "charging_full_check,BMT_status.bat_vol=%d,gFG_current=%d,start disable charging on\n",BMT_status.bat_vol,gFG_current);
		}
	} else {
		full_check_count = 0;
		battery_log(BAT_LOG_CRTI,
				    "charging_full_check not full,BMT_status.bat_vol=%d,gFG_current=%d,start disable charging on\n",BMT_status.bat_vol,gFG_current);
	}
	return status;
...
void pchr_turn_on_charging(void)
{
	...
	if (BMT_status.bat_charging_state == CHR_ERROR) {
		...
		if (get_usb_current_unlimited()) {
			...
			select_charging_current();
			battery_log(BAT_LOG_CRTI, "[BATTERY] select_charging_curret !Default CC mode charging = %d, input current = %d \n",g_temp_CC_value,g_temp_input_CC_value);
			save_CC_value =g_temp_CC_value;
			save_input_CC_value =g_temp_input_CC_value;

			if (g_bcct_flag == 1 ||g_bcct_input_flag==1) {
			//select_charging_current();
			select_charging_current_bcct();
			battery_log(BAT_LOG_FULL, "[BATTERY] select_charging_curret_bcct !CC  charging = %d, input current = %d \n",g_temp_CC_value,g_temp_input_CC_value);
			}
			if(g_temp_CC_value >= save_CC_value)
				{
					g_temp_CC_value = save_CC_value;
					battery_log(BAT_LOG_FULL, "[BATTERY] select_charging_curret ,g_temp_CC_value=%d\n",g_temp_CC_value);
				}		
			if(g_temp_input_CC_value >= save_input_CC_value)
				{
					g_temp_input_CC_value = save_input_CC_value;
					battery_log(BAT_LOG_FULL, "[BATTERY] select_charging_curret ,g_temp_input_CC_value=%d\n",g_temp_input_CC_value);
			        }
		} 
#endif
		battery_log(BAT_LOG_CRTI,
			    "[BATTERY] Mid g_temp_CC_value=%d,g_temp_input_CC_value=%d\r\n",		    
			    g_temp_CC_value, g_temp_input_CC_value);
		if((g_call_state==1)&&(g_lcm_status==0))
		{
				battery_log(BAT_LOG_FULL, "g_call_state=%d,g_lcm_status=%d,g_temp_input_CC_value=%d,g_temp_CC_value=%d\n",g_call_state,g_lcm_status,g_temp_input_CC_value,g_temp_CC_value);
			        if(g_temp_input_CC_value > CHARGE_CURRENT_900_00_MA)
			        {
			        	g_temp_input_CC_value = CHARGE_CURRENT_900_00_MA;
						g_temp_CC_value = CHARGE_CURRENT_900_00_MA;
						battery_log(BAT_LOG_FULL, "1g_call_state=%d,g_lcm_status=%d,g_temp_input_CC_value=%d,g_temp_CC_value=%d\n",g_call_state,g_lcm_status,g_temp_input_CC_value,g_temp_CC_value);
			        }	
		}
		...
		if(g_lcm_status==1)
		{
				...
				battery_log(BAT_LOG_FULL, "g_lcm_status=%d,g_temp_input_CC_value=%d,g_temp_CC_value=%d\n",g_lcm_status,g_temp_input_CC_value,g_temp_CC_value);

			        if(g_temp_input_CC_value > CHARGE_CURRENT_1500_00_MA)
			        {
			        	g_temp_input_CC_value = CHARGE_CURRENT_1500_00_MA;
						g_temp_CC_value = CHARGE_CURRENT_1200_00_MA;
						battery_log(BAT_LOG_FULL, "1g_lcm_status=%d,g_temp_input_CC_value=%d,g_temp_CC_value=%d\n",g_lcm_status,g_temp_input_CC_value,g_temp_CC_value);
			        }	
		...
		if (g_temp_CC_value == CHARGE_CURRENT_0_00_MA
			...
			battery_log(BAT_LOG_CRTI,
			    "[BATTERY] Final CC mode charging : %d, input current = %d\r\n",
			    g_temp_CC_value, g_temp_input_CC_value);
...
PMU_STATUS BAT_BatteryFullAction(void)
{
	...
	charging_enable = KAL_FALSE;
	battery_charging_control(CHARGING_CMD_ENABLE, &charging_enable);
	//battery_vol=battery_meter_get_battery_voltage(KAL_TRUE);	
