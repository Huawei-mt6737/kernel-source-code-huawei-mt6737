...
#include <linux/delay.h>
#define CLIENT_NAME_POWER_SM5414		"dsm_charger"
static int rgb_sm5414_debug_mask= 1;
...
static struct dsm_dev dsm_power_sm5414 = {
	.name 		= CLIENT_NAME_POWER_SM5414,		// dsm client name
	.fops 		= NULL,						      // options
	.buff_size 	= DSM_SENSOR_BUF_MAX,			// buffer size
};
#define SM5414_ERR(x...) do {\
    if (rgb_sm5414_debug_mask >=0) \
        printk(KERN_ERR x);\
    } while (0)

#define SM5414_INFO(x...) do {\
    if (rgb_sm5414_debug_mask >=1) \
        printk(KERN_ERR x);\
    } while (0)
#define SM5414_FLOW(x...) do {\
    if (rgb_sm5414_debug_mask >=2) \
        printk(KERN_ERR x);\
    } while (0)
static struct dsm_client *sm5414_power_dclient = NULL;
static int sm5414_dsm_report_err(int errno)
{
	int size = 0;
	if (FACTORY_BOOT == get_boot_mode() )
	return 0;
	if(dsm_client_ocuppy(sm5414_power_dclient)){
		/* buffer is busy */
		SM5414_ERR("%s: buffer is busy!, errno = %d\n", __func__,errno);
		return -EBUSY;
	}

	SM5414_INFO("dsm error, errno = %d, vbus_dsm:%d,vbat_dsm:%d,capacity_dsm:%d,present_dsm:%d \
		   charge_current:%d \n", errno,battery_vbus_dsm,battery_vbat_dsm,battery_capacity_dsm,
		   battery_present_dsm,battery_charge_current);

	size = dsm_client_record(sm5414_power_dclient,
	"vbus:%d,vbat:%d,capacity:%d,present:%d,,temp:%d\n"
	,battery_vbus_dsm,battery_vbat_dsm,battery_capacity_dsm,battery_present_dsm,battery_temp_dsm);

	/*if device is not probe successfully or client is null, don't notify dsm work func*/


	dsm_client_notify(sm5414_power_dclient, errno);
...
#ifdef CONFIG_HUAWEI_DSM
if(BMT_status.charger_type==3)
sm5414_dsm_report_err(DSM_CHARGER_NON_STANDARD);
#endif
}
...
battery_vbus_dsm=battery_main.BAT_ChargerVoltage;
battery_vbat_dsm=battery_main.BAT_batt_vol;
battery_capacity_dsm=battery_main.BAT_CAPACITY;
battery_temp_dsm=battery_main.BAT_batt_temp;
battery_present_dsm=battery_main.BAT_PRESENT;
battery_ac_dsm=ac_main.AC_ONLINE;
battery_usb_dsm=usb_main.USB_ONLINE;
battery_charge_current=battery_main.BAT_BatteryAverageCurrent;
}
int mt_battery_charge_status_dsm(void)
{
if((battery_ac_dsm ||battery_usb_dsm) && (battery_vbat_dsm<4100) &&(battery_charge_current<20))
...
int bat_routine_thread(void *x)
{
	...
	while (1) {
...
#ifdef CONFIG_HUAWEI_DSM
#if 0
		if(1==mt_battery_charge_status_dsm())
		sm5414_dsm_report_err(DSM_CHARGER_ERROR_NO);
#endif
		if(battery_vbus_dsm>6000)
		sm5414_dsm_report_err(DSM_CHARGER_OVP_NOERROR_NO);
		if(!battery_present_dsm)
		sm5414_dsm_report_err(DSM_BATTERY_EXIST_NOERROR_NO);
		temp_capacity=battery_capacity_dsm;
		if(i==5)
		{
		first_capacity=battery_capacity_dsm;
		}
		i++;
		if(i>=11)
		{
		...
		battery_log(BAT_LOG_CRTI, "wait event 1\n");
		   if(((temp_capacity-first_capacity)>5 || (temp_capacity-first_capacity<-5))&&(first_capacity!=0))
		   	{
		   	SM5414_ERR("dsm capcity error first capacity:%d,last capacity:%d \n", first_capacity,temp_capacity);

		sm5414_dsm_report_err(DSM_CHARGER_CAPACITY_ERROR_NO);
		   	}
#if 0
          if ((((upmu_is_chr_det() == KAL_TRUE) && (battery_ac_dsm) )
|| ((upmu_is_chr_det() == KAL_TRUE) && (battery_usb_dsm) ) || ((upmu_is_chr_det() == KAL_FALSE) && (!battery_ac_dsm) ) ||((upmu_is_chr_det() == KAL_FALSE) && (!battery_ac_dsm) ))==0)
            sm5414_dsm_report_err(DSM_CHARGE_DETECT_NOERROR_NO);
#endif
#endif
...
int bat_update_thread_dsm(void *x)
{
      int i=0;
      int temp_capacity=0;
      int first_capacity=0;
	...
	while (1) {
		...
		i=0;
		   ...
		   if((temp_capacity-first_capacity)>5)
		sm5414_dsm_report_err(DSM_CHARGER_CAPACITY_ERROR_NO);
          if ((((upmu_is_chr_det() == KAL_TRUE) && (battery_ac_dsm) )
|| ((upmu_is_chr_det() == KAL_TRUE) && (battery_usb_dsm) ) || ((upmu_is_chr_det() == KAL_FALSE) && (!battery_ac_dsm) ) ||((upmu_is_chr_det() == KAL_FALSE) && (!battery_ac_dsm) ))==0)
            sm5414_dsm_report_err(DSM_CHARGE_DETECT_NOERROR_NO);
...
#ifdef CONFIG_HUAWEI_DSM
	sm5414_power_dclient = dsm_register_client(&dsm_power_sm5414);
	if(!sm5414_power_dclient)
	SM5414_ERR("%s: register dsm_dclient failed!!, line = %d\n", __func__,__LINE__);
#endif
...
#if 0
#ifdef CONFIG_HUAWEI_DSM
	kthread_run(bat_update_thread_dsm, NULL, "bat_dsm");
#endif
#endif
...
static int battery_remove(struct platform_device *dev)
{
	...
	if(sm5414_power_dclient)
		{
	dsm_unregister_client(sm5414_power_dclient,&dsm_power_sm5414);
	sm5414_power_dclient=NULL;
		}
	#endif
	return 0;
}
