...
#define CLIENT_NAME_AUDIO_CODEC	"dsm_audio_info"

static int rgb_codec_debug_mask = 1;
static struct dsm_dev dsm_audio_codec = {
	.name 		= CLIENT_NAME_AUDIO_CODEC,		// dsm client name
	.fops 		= NULL,						      // options
	.buff_size 	= DSM_SENSOR_BUF_MAX,			// buffer size
};
#define CODEC_ERR(x...) do {\
    if (rgb_codec_debug_mask >=0) \
        printk(KERN_ERR x);\
    } while (0)

#define CODEC_INFO(x...) do {\
    if (rgb_codec_debug_mask >=1) \
        printk(KERN_ERR x);\
    } while (0)
#define CODEC_FLOW(x...) do {\
    if (rgb_codec_debug_mask >=2) \
        printk(KERN_ERR x);\
    } while (0)
static struct dsm_client *codec_audio_dclient = NULL;
static int codec_dsm_report_err(int errno, int type)
{
	int size = 0;
	...
	// 1st
	if (dsm_client_ocuppy(codec_audio_dclient)) {
		/* buffer is busy */
		CODEC_ERR("%s: buffer is busy!, errno = %d\n", __func__,errno);
		return -EBUSY;
	}

	// 2nd
	CODEC_INFO("dsm error, errno = %d \n", errno);
    if (type == 0)
    	size = dsm_client_record(codec_audio_dclient, "Ext_Speaker_Amp_Change GetGPIO_Info FAIL\n");
    else if (type == 1)
    	size = dsm_client_record(codec_audio_dclient, "Receiver_Speaker_Switch_Change GetGPIO_Info FAIL\n");
    else if (type == 2)
    	size = dsm_client_record(codec_audio_dclient, "Headset_PGAL_Set  EINVAL\n");
    else if (type == 3)
    	size = dsm_client_record(codec_audio_dclient, "Headset_PGAR_Set  EINVAL\n");
    else if (type == 4)
    	size = dsm_client_record(codec_audio_dclient, "Audio_ADC1_Sel_Set  EINVAL\n");
    else if (type == 5)
    	size = dsm_client_record(codec_audio_dclient, "Audio_ADC2_Sel_Set  EINVAL\n");
    else if (type == 6)
    	size = dsm_client_record(codec_audio_dclient, "Audio_PreAmp1_Set  EINVAL\n");
    else if (type == 7)
    	size = dsm_client_record(codec_audio_dclient, "Audio_PreAmp2_Set  EINVAL\n");
    else if (type == 8)
    	size = dsm_client_record(codec_audio_dclient, "Audio_PGA1_Set  EINVAL\n");
    else if (type == 9)
    	size = dsm_client_record(codec_audio_dclient, "Audio_PGA2_Set  EINVAL\n");
    else if (type == 10)
    	size = dsm_client_record(codec_audio_dclient, "Audio_MicSource1_Set  EINVAL\n");
    else if (type == 11)
    	size = dsm_client_record(codec_audio_dclient, "Audio_Mic1_Mode_Select_Set  EINVAL\n");
    else if (type == 12)
    	size = dsm_client_record(codec_audio_dclient, "Audio_Mic2_Mode_Select_Set  EINVAL\n");
    else if (type == 13)
    	size = dsm_client_record(codec_audio_dclient, "Audio_Mic3_Mode_Select_Set  EINVAL\n");
    else if (type == 14)
    	size = dsm_client_record(codec_audio_dclient, "Audio_Mic4_Mode_Select_Set  EINVAL\n");
    else if (type == 15)
    	size = dsm_client_record(codec_audio_dclient, "Audio_Adc_Power_Mode_Set  EINVAL\n");
    else if (type == 16)
    	size = dsm_client_record(codec_audio_dclient, "Audio_Vow_ADC_Func_Switch_Set  EINVAL\n");
    else if (type == 17)
    	size = dsm_client_record(codec_audio_dclient, "Audio_Vow_Digital_Func_Switch_Set  EINVAL\n");
    else if (type == 18)
    	size = dsm_client_record(codec_audio_dclient, "Audio_Vow_MIC_Type_Select_Set  EINVAL\n");
    else if (type == 19)
    	size = dsm_client_record(codec_audio_dclient, "Pmic_Loopback_Set  EINVAL\n");
	/*if device is not probe successfully or client is null, don't notify dsm work func*/

	// 3rd
	dsm_client_notify(codec_audio_dclient, errno);
...
#else
		ret += AudDrv_GPIO_EXTAMP_Select(false);
...
#else
		ret += AudDrv_GPIO_EXTAMP_Select(true);
		udelay(1);
		ret += AudDrv_GPIO_EXTAMP_Select(false);
		udelay(1);
		ret +=AudDrv_GPIO_EXTAMP_Select(true);
...
#ifdef CONFIG_HUAWEI_DSM
	if (ret)
        	codec_dsm_report_err(DSM_CODEC_SPEAKER_ERROR, 0);
#endif
#endif
}
...
static void Receiver_Speaker_Switch_Change(bool enable)
{
	...
	if (enable) {
		...
		if (ret < 0) {
#ifdef CONFIG_HUAWEI_DSM
			codec_dsm_report_err(DSM_CODEC_SPEAKER_ERROR, 1);
#endif
...
#ifdef CONFIG_HUAWEI_DSM
		codec_dsm_report_err(DSM_CODEC_ERROR, 2);
#endif
...
static int Headset_PGAR_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.enumerated.item[0] >= ARRAY_SIZE(DAC_DL_PGA_Headset_GAIN)) {
#ifdef CONFIG_HUAWEI_DSM
		codec_dsm_report_err(DSM_CODEC_ERROR, 3);
#endif
		pr_err("return -EINVAL\n");
		return -EINVAL;
	}
...
#ifdef CONFIG_HUAWEI_DSM
		codec_dsm_report_err(DSM_CODEC_ERROR, 4);
#endif
		pr_err("return -EINVAL\n");
		return -EINVAL;
	}
...
static int Audio_ADC2_Sel_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.enumerated.item[0] > ARRAY_SIZE(Adc_Input_Sel)) {
#ifdef CONFIG_HUAWEI_DSM
		codec_dsm_report_err(DSM_CODEC_ERROR, 5);
#endif
		pr_err("return -EINVAL\n");
		return -EINVAL;
	}
...
#ifdef CONFIG_HUAWEI_DSM
		codec_dsm_report_err(DSM_CODEC_ERROR, 6);
#endif
		pr_err("return -EINVAL\n");
		return -EINVAL;
	}
...
static int Audio_PreAmp2_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.enumerated.item[0] > ARRAY_SIZE(PreAmp_Mux_function)) {
#ifdef CONFIG_HUAWEI_DSM
		codec_dsm_report_err(DSM_CODEC_ERROR, 7);
#endif
		pr_err("return -EINVAL\n");
		return -EINVAL;
	}
...
#ifdef CONFIG_HUAWEI_DSM
		codec_dsm_report_err(DSM_CODEC_ERROR, 8);
#endif
		pr_err("return -EINVAL\n");
		return -EINVAL;
	}
...
static int Audio_PGA2_Set(struct snd_kcontrol *kcontrol, struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.enumerated.item[0] > ARRAY_SIZE(ADC_UL_PGA_GAIN)) {
#ifdef CONFIG_HUAWEI_DSM
		codec_dsm_report_err(DSM_CODEC_ERROR, 9);
#endif
		pr_err("return -EINVAL\n");
		return -EINVAL;
	}
...
#ifdef CONFIG_HUAWEI_DSM
		codec_dsm_report_err(DSM_CODEC_ERROR, 10);
#endif
		pr_err("return -EINVAL\n");
		return -EINVAL;
	}
...
#ifdef CONFIG_HUAWEI_DSM
		codec_dsm_report_err(DSM_CODEC_ERROR, 11);
#endif
		pr_err("return -EINVAL\n");
		return -EINVAL;
	}
				      ...
				      struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.enumerated.item[0] > ARRAY_SIZE(Audio_AnalogMic_Mode)) {
#ifdef CONFIG_HUAWEI_DSM
		codec_dsm_report_err(DSM_CODEC_ERROR, 12);
#endif
		pr_err("return -EINVAL\n");
		return -EINVAL;
	}
				      ...
				      struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.enumerated.item[0] > ARRAY_SIZE(Audio_AnalogMic_Mode)) {
#ifdef CONFIG_HUAWEI_DSM
		codec_dsm_report_err(DSM_CODEC_ERROR, 13);
#endif
		pr_err("return -EINVAL\n");
		return -EINVAL;
	}
				      ...
				      struct snd_ctl_elem_value *ucontrol)
{
	...
	if (ucontrol->value.enumerated.item[0] > ARRAY_SIZE(Audio_AnalogMic_Mode)) {
#ifdef CONFIG_HUAWEI_DSM
		codec_dsm_report_err(DSM_CODEC_ERROR, 14);
#endif
		pr_err("return -EINVAL\n");
		return -EINVAL;
	}
...
#ifdef CONFIG_HUAWEI_DSM
		codec_dsm_report_err(DSM_CODEC_ERROR, 15);
#endif
		pr_err("return -EINVAL\n");
		return -EINVAL;
	}
...
#ifdef CONFIG_HUAWEI_DSM
		codec_dsm_report_err(DSM_CODEC_ERROR, 16);
#endif
		pr_err("return -EINVAL\n");
		return -EINVAL;
	}
...
#ifdef CONFIG_HUAWEI_DSM
		codec_dsm_report_err(DSM_CODEC_ERROR, 17);
#endif
		pr_err("return -EINVAL\n");
		return -EINVAL;
	}
...
#ifdef CONFIG_HUAWEI_DSM
		codec_dsm_report_err(DSM_CODEC_ERROR, 18);
#endif
		pr_err("return -EINVAL\n");
		return -EINVAL;
	}
...
#ifdef CONFIG_HUAWEI_DSM
		codec_dsm_report_err(DSM_CODEC_ERROR, 19);
#endif
		pr_err("return -EINVAL\n");
		return -EINVAL;
	}
...
#ifdef CONFIG_HUAWEI_DSM
	codec_audio_dclient = dsm_register_client(&dsm_audio_codec);
	if (NULL == codec_audio_dclient)
		CODEC_ERR("%s: register dsm_dclient failed!!, line = %d\n", __func__, __LINE__);
#endif
...
#ifdef CONFIG_HUAWEI_DSM
	if (codec_audio_dclient) {
		dsm_unregister_client(codec_audio_dclient, &dsm_audio_codec);
		codec_audio_dclient = NULL;
	}
#endif
