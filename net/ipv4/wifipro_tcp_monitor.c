        ...
        if (!strncmp(sk->wifipro_dev_name, "wlan", 4)) {
            ...
            if (skb->dev) {
                if (!strncmp(skb->dev->name, "wlan", 4)) {
                    ...
                    unknown_dev_count++;
                    WIFIPRO_DEBUG("unknown out device, %s -> %s state is %d",
                            wifipro_ntoa(src_addr), wifipro_ntoa(dest_addr), sk->sk_state);
                    return;
                }
            } else {
                unknown_dev_count++;
        ...
        if (!strncmp(sk->wifipro_dev_name, "wlan", 4)) {
            ...
            if (skb->dev) {
                if (!strncmp(skb->dev->name, "wlan", 4)) {
                    ...
                    unknown_dev_count++;
                    WIFIPRO_DEBUG("unknown in device, %s -> %s state is %d",
                            wifipro_ntoa(src_addr), wifipro_ntoa(dest_addr), sk->sk_state);
                    return;
                }
            } else {
...
bool wifipro_is_trigger_sock(unsigned int dest_addr, unsigned int dest_port) {
    bool ret = false;
    if (is_delayed_work_handling && wifipro_trigger_sock->dst_addr == dest_addr
...
static bool wifipro_is_cong_occured(wifipro_cong_sock_t *src,
    ...
    end: mutex_unlock(&wifipro_congestion_sem);
    return ret;
}
    ...
    if (is_wifipro_running && !is_delayed_work_handling
        ...
        if (wifipro_tcp_trigger_inf) {
            ...
            wifipro_tcp_trigger_inf->InSegs =
                    snmp_fold_field(
                            (void __percpu **) (wifipro_retrans_sock->net)->mib.wifipro_tcp_statistics,
                            WIFIPRO_TCP_MIB_WLAN_INSEGS);
...
static int wifipro_tcp_monitor_send_msg(int pid, unsigned int msg_from,
    ...
    packet->tcp_rx_pkts = snmp_fold_field(
            (void __percpu **) init_net.mib.wifipro_tcp_statistics,
            WIFIPRO_TCP_MIB_WLAN_INSEGS);
    packet->tcp_tx_pkts = snmp_fold_field(
            (void __percpu **) init_net.mib.wifipro_tcp_statistics,
            WIFIPRO_TCP_MIB_WLAN_OUTSEGS);
    packet->tcp_retrans_pkts = snmp_fold_field(
            (void __percpu **) init_net.mib.wifipro_tcp_statistics,
            WIFIPRO_TCP_MIB_WLAN_RETRANSSEGS);
    ...
    end: mutex_unlock(&wifipro_nl_send_sem);
...
static int wifipro_notify_rtt_msg(int pid, unsigned int msg_from) {
    ...
    case WIFIPRO_NOTIFY_WLAN_BQE_RTT:
        packet->rtt = (wlan_bqe_rtt_stat.rtt / wlan_bqe_rtt_stat.packets)
                * WIFIPRO_TICK_TO_MS;
    ...
    case WIFIPRO_NOTIFY_MOBILE_BQE_RTT:
        packet->rtt = (mobile_bqe_rtt_stat.rtt / mobile_bqe_rtt_stat.packets)
                * WIFIPRO_TICK_TO_MS;
    ...
    case WIFIPRO_NOTIFY_WLAN_SAMPLE_RTT:
        packet->rtt = (wlan_sample_rtt_stat.rtt / wlan_sample_rtt_stat.packets)
                * WIFIPRO_TICK_TO_MS;
    ...
    if (skb->len >= NLMSG_HDRLEN) {
        ...
        if ((nlh->nlmsg_len >= sizeof(struct nlmsghdr))
            ...
            case NETLINK_WIFIPRO_GET_MSG:
                wifipro_tcp_monitor_send_msg(nlh->nlmsg_pid, WIFIPRO_APP_QUERY,
                        LINK_UNKNOWN);
                break;
...
static void wifipro_tcp_monitor_work_handler(struct work_struct *work) {
    ...
    wifipro_tcp_curr_inf.InSegs = snmp_fold_field(
            (void __percpu **) init_net.mib.wifipro_tcp_statistics,
            WIFIPRO_TCP_MIB_WLAN_INSEGS);
    wifipro_tcp_curr_inf.OutSegs = snmp_fold_field(
            (void __percpu **) init_net.mib.wifipro_tcp_statistics,
            WIFIPRO_TCP_MIB_WLAN_OUTSEGS);
    wifipro_tcp_curr_inf.RetransSegs = snmp_fold_field(
            (void __percpu **) init_net.mib.wifipro_tcp_statistics,
            WIFIPRO_TCP_MIB_WLAN_RETRANSSEGS);
    wifipro_tcp_curr_inf.InErrs = snmp_fold_field(
            (void __percpu **) init_net.mib.wifipro_tcp_statistics,
            WIFIPRO_TCP_MIB_INERRS);

    mutex_lock(&wifipro_tcp_trigger_inf_sem);
    if (wifipro_tcp_trigger_inf) {
        diff_OutSegs = wifipro_tcp_curr_inf.OutSegs
                - wifipro_tcp_trigger_inf->OutSegs;
        if (0 != diff_OutSegs) {
            retrans_rate = (wifipro_tcp_curr_inf.RetransSegs
    ...
    if (wifipro_tcp_trigger_inf && wifipro_trigger_sock) {
        Interval_OutSegs = wifipro_tcp_curr_inf.OutSegs
                - wifipro_tcp_trigger_inf->OutSegs
                - wifipro_trigger_sock->OutSegs;
        Interval_RetransSegs = wifipro_tcp_curr_inf.RetransSegs
                - wifipro_tcp_trigger_inf->RetransSegs
    ...
    mutex_lock(&wifipro_tcp_trigger_inf_sem);
    if (wifipro_tcp_trigger_inf) {
        Interval_InSegs = wifipro_tcp_curr_inf.InSegs
                - wifipro_tcp_trigger_inf->InSegs;
    }
    mutex_unlock(&wifipro_tcp_trigger_inf_sem);
    ...
    if (Interval_InSegs >= 80) {
    ...
    } else if (wlan_rtt_average
            <= 1000&& wlan_rtt_calc_pkg > 3 && time_after(wlan_when_recorded_rtt + WIFIPRO_MONITOR_DELAY, jiffies)) {tcp_quality
        = LINK_MAYBE_GOOD;
        ...
        if (wlan_rtt_average >= WIFIPRO_RTT_THRESHOLD
                && wlan_rtt_calc_pkg
                        > 5&& time_after(wlan_when_recorded_rtt + WIFIPRO_MONITOR_DELAY, jiffies)) {
            if (wifipro_is_cong_occured(wifipro_congestion_stat,
                    TCP_CA_Disorder)
    ...
    if (tcp_quality == LINK_MAYBE_POOR || tcp_quality == LINK_POOR) {
        wifipro_tcp_monitor_send_msg(g_user_space_pid, WIFIPRO_KNL_NOTIFY,
                tcp_quality);
    }
...
void wifipro_handle_congestion(struct sock *sk, u8 ca_state) {
    ...
    switch (ca_state) {
    case TCP_CA_Open:
        wifipro_set_cong_stat(dest_addr, dest_port, wifipro_congestion_stat,
                TCP_CA_Open);
        break;
...
static void wifipro_update_wlan_rtt(unsigned int rtt, struct sock *sk,
    ...
    WIFIPRO_DEBUG("-->%s wlan: rto=%d  srtt_us=%d  pks=%d, rtt_avg=%d",
            wifipro_ntoa(dest_addr), icsk->icsk_rto, tp->srtt_us, wlan_rtt_calc_pkg, wlan_rtt_average);
}
...
static void wifipro_update_mobile_rtt(unsigned int rtt, struct sock *sk,
    ...
    WIFIPRO_DEBUG("-->%s mobile: rto=%d  srtt_us=%d  pkts=%d, rtt_avg=%d",
            wifipro_ntoa(dest_addr), icsk->icsk_rto, tp->srtt_us, mobile_rtt_calc_pkg, mobile_rtt_average);
}
...
static wifipro_rtt_second_stat_t* wifipro_rtt_init(bool is_wlan) {
    ...
    wifipro_rtt_second_stat_head = kzalloc(sizeof(wifipro_rtt_second_stat_t),
            GFP_KERNEL);
