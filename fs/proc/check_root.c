...
/**define**********************************************************************/
#define STORE_FILE      "/dev/block/platform/mtk-msdc.0/11230000.msdc0/by-name/proinfo"
#define BASE_ADDR (0)
...
static int root_setids_status_save(void);
static int checkroot_risk_id(int curr_id, int flag)
{
	...
	if(checkroot_ref.setuid !=0 || checkroot_ref.setgid !=0 || checkroot_ref.setresuid !=0 || checkroot_ref.setresgid !=0)
	{
                //rtc_mark_rootcheck();
                res = root_setids_status_save();
                if(res < 0)
                {
                      printk("save setids status to proinfo fail\n");
                };
                printk("checkroot_risk_id,uid has changed,save to proinfo!\n");
	}
	return 0;
#endif
}
...
static int root_setids_status_save(void)
{
        ...
        tmp_buf = kzalloc(1, GFP_KERNEL);
        if(!tmp_buf){
        res = -1;
        goto err1;
        }

        //err = copy_from_user(tmp_buf,in_buf,1024);
        *tmp_buf=2;
        ...
        printk("private_slot_write open file sucess\n");

        f->f_pos=BASE_ADDR + 773;
        fs = get_fs();
        set_fs(KERNEL_DS);
        res = f->f_op->write(f, tmp_buf, 1024, &f->f_pos);
        set_fs(fs);
